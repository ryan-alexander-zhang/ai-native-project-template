 ...
 *
 * <p>The quantity is what makes the answer worth asking for: the previous shape carried SKUs alone,
 * so the gate could only answer "is any on hand?" — a 999-unit order sailed through against a stock
 * of 5 and was placed only to walk the whole compensation circle. Asking with the quantity turns
 * the same synchronous check into "can you offer <em>this many</em>?". Lines repeating a SKU are
 * summed by the answering side, so the verdict is about the caller's total demand.
 */
public record StockQuery(List<Line> lines) {

  public StockQuery {
    // Refused at parse time: a null list is a malformed request, not a question

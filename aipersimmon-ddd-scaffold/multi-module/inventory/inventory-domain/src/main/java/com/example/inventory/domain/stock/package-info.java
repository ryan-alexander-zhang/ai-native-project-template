 ...
 * decision, not an omission. A domain event exists for subscribers <em>inside</em> the context:
 * ordering has them (its fulfilment trigger reacts to {@code OrderReadyForFulfilment} becoming
 * true), so its aggregates announce. Inventory has none — nothing in this context reacts to "stock
 * was reserved" except the very handler that reserved it, which then assembles the cross-context
 * integration event directly. Recording a domain event here would add a hop (aggregate → drain →
 * subscriber → integration event) whose only subscriber restates what the handler already knows, to
 * no reader's benefit.
 *

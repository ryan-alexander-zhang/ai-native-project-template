...
/**
 * Answers {@link CheckStockAvailability} by reading the current level of each SKU from
 * the {@link Stocks} repository. A SKU with no stock record reports {@code available = 0},
 * so an unknown SKU and an out-of-stock one look the same to the caller — enough for an
 * offerability check. Read-only: it neither reserves nor mutates stock, which is why it
 * is a query handler with no {@code CommandContext} and no events.
 */
@Component
@UseCase

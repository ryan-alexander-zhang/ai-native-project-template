...
public class ReleaseStockHandler implements CommandHandler<ReleaseStock, Void> {
    ...
    public ReleaseStockHandler(Stocks stocks, Reservations reservations, IntegrationEvents integrationEvents) {
        this.stocks = stocks;
    ...
    public Void handle(ReleaseStock command, CommandContext context) {
        Reservation reservation = reservations.findById(new ReservationId(command.reservationId()))
                .orElseThrow(() -> new DomainException(
                        InventoryErrorCode.RESERVATION_NOT_FOUND, "unknown reservation: " + command.reservationId()));
        ...
        if (reservation.markReleased()) {
            for (Map.Entry<Sku, Integer> line : reservation.held()) {
                Stock stock = stocks.findBySku(line.getKey())
                        .orElseThrow(() -> new DomainException(
                                InventoryErrorCode.STOCK_NOT_FOUND, "unknown sku: " + line.getKey().value()));
                stock.release(line.getValue());
                stocks.save(stock);
            }
            reservations.save(reservation);
        }
        integrationEvents.publish(new StockReleased(reservation.orderId(), command.reservationId()), context);
        return null;
    }
}

...
/**
 * Real middleware for the full-context acceptance tests via Testcontainers. {@code @ServiceConnection}
 * wires the app's DataSource to the PostgreSQL container and its {@code spring.kafka.bootstrap-servers}
 * to the Kafka container — the same two middleware the app runs against via docker-compose in dev.
 * The aipersimmon Flyway starter creates the process-manager, outbox, and inbox tables on the
 * container at startup. Imported by every {@code @SpringBootTest} in this module.
 *

...
/**
 * Reacts to ordering's {@link PaymentRequested} integration event by sending an {@link
 * AuthorizePayment} command through the command bus. As the payment context's anti-corruption layer
 * it reads only ordering's published contract and preserves the causing event's context, so the
 * {@code PaymentAuthorized}/{@code PaymentDeclined} it triggers stays correlated to the order.
 */
@Component

...
/**
 * Reacts to ordering's {@link PaymentRequested} integration event by sending a {@link ChargePayment}
 * command through the command bus. As the payment context's anti-corruption layer it reads only
 * ordering's published contract and preserves the causing event's context, so the

 ...
 * {@code ordering.order_lines}. Runs inside the command's transaction (the CQRS
 * {@code TransactionCommandInterceptor}) on the same DataSource as the outbox, so the aggregate and
 * its integration event commit atomically. save() rewrites the lines wholesale (delete + insert) —
 * an order's line set is small and only set at placement.
 */
@Repository
public class MyBatisOrders implements Orders {
    ...
    public Optional<Order> findById(OrderId id) {
        ...
        List<OrderLineDo> rows = lines.selectList(new LambdaQueryWrapper<OrderLineDo>()
        ...
        for (OrderLineDo row : rows) {
            lineData.add(new LineData(row.getSku(), row.getQuantity(),
                    Money.of(row.getUnitMinor(), row.getCurrency())));
        }
        return Optional.of(Order.reconstitute(
                id, new CustomerId(header.getCustomerId()), lineData, OrderStatus.valueOf(header.getStatus())));
    }
}

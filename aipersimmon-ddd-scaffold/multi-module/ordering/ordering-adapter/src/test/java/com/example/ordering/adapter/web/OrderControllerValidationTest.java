...
/**
 * Edge-tier validation slice: drives {@link OrderController} through MockMvc with the command/query
 * buses mocked, so it verifies only the web concern — that {@code @Valid} on the request DTO
 * rejects a malformed body with 400 before any command is dispatched. The command bus's own
 * validation is a separate, non-web concern tested at the application layer.
 */

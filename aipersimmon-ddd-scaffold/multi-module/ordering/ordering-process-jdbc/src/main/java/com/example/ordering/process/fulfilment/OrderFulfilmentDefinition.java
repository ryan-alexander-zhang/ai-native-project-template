 ...
 * {@link #react} gates every fact on the {@linkplain OrderFulfilmentState#step() current step}, not on
 * the input type alone. For each {@code (step, input)} pair it does exactly one of:
 * <ul>
 ...
 *       is a duplicate of one already handled, or is out of order for this step. Because the runtime
 *       delivers at-least-once and treats a {@code react} throw as a poison message it retries
 *       forever, an out-of-order or stale fact must <em>not</em> throw — it is ignored. This closes
 *       the three misbehaviours a type-only switch had: {@code PaymentAuthorized} at
 *       {@code AWAITING_STOCK} no longer confirms an un-reserved order; {@code PaymentDeclined} before
 *       a reservation no longer releases a {@code null} handle; {@code StockReleased} before a decline
 *       no longer throws on a {@code null} decline code and wedges the queue;
 *   <li><b>reject</b> — a throw, reserved for {@link OrderFulfilmentInput.OrderPlaced}, which is a
 *       start-only input that structurally never reaches {@code react} (so the throw cannot poison a
 *       real redelivery) and only signals a wiring defect.
 * </ul>
 *
 * <p>Evidence refs carry the identity of the <em>causing envelope</em>
 * ({@code context.cause().messageId()}), not a business key: the reservation-failure, payment-decline,
 * and stock-release ids are each the id of the event that produced them, so they are distinct and
 * traceable rather than colliding on {@code orderId}/{@code reservationId}. The decline event's id is
 * remembered in the state when payment declines, so the eventual cancellation names the true decline
 * event even though the current cause is the stock-released event.
 */
@Component
public class OrderFulfilmentDefinition implements ProcessDefinition<OrderFulfilmentState> {
    ...
    private ProcessDecision<OrderFulfilmentState> onAwaitingStock(
        ...
        if (in instanceof OrderFulfilmentInput.StockReserved reserved) {
            ...
            return running(
                    ...
                    "stock-reserved", new DispatchCommand(new RequestPayment(orderId, paymentOperationId)));
        }
        if (in instanceof OrderFulfilmentInput.StockReservationFailed failed) {
            ReservationFailureRef failure = new ReservationFailureRef(
                    context.cause().messageId(), new OrderId(state.orderId()), failed.code(), failed.reason());
            return compensating(
                    state.withStep(Step.AWAITING_ORDER_CANCELLATION), Step.AWAITING_ORDER_CANCELLATION,
                    "stock-reservation-failed",
                    new DispatchCommand(new CancelOrder(state.orderId(), new CancellationReason.InventoryUnavailable(failure))));
        }
    ...
    private ProcessDecision<OrderFulfilmentState> onAwaitingPayment(
        ...
        if (in instanceof OrderFulfilmentInput.PaymentAuthorized) {
            return running(
                    ...
                    "payment-authorized", new DispatchCommand(new ConfirmOrder(state.orderId())));
        }
        if (in instanceof OrderFulfilmentInput.PaymentDeclined declined) {
            return compensating(
                    ...
                    Step.AWAITING_STOCK_RELEASE, "payment-declined",
    ...
    private ProcessDecision<OrderFulfilmentState> onAwaitingStockRelease(
        ...
        if (in instanceof OrderFulfilmentInput.StockReleased released) {
            ...
            CancellationReason reason = new CancellationReason.PaymentDeclinedAfterStockReleased(
                    new PaymentDeclineRef(state.paymentDeclineEvidenceId(), id, state.paymentDeclineCode()),
                    new StockReleaseRef(context.cause().messageId(), id));
            return compensating(
                    state.withStep(Step.AWAITING_ORDER_CANCELLATION), Step.AWAITING_ORDER_CANCELLATION,
                    "stock-released", new DispatchCommand(new CancelOrder(state.orderId(), reason)));
        }
        return ignore(state, in, context);
    }
    ...
    private ProcessDecision<OrderFulfilmentState> onAwaitingOrderConfirmation(
        ...
        if (in instanceof OrderFulfilmentInput.OrderConfirmed) {
            return completed(state.withStep(Step.CONFIRMED), Step.CONFIRMED, "order-confirmed", "ORDER_CONFIRMED");
        }
        return ignore(state, in, context);
    }
    ...
    private ProcessDecision<OrderFulfilmentState> onAwaitingOrderCancellation(
        ...
        if (in instanceof OrderFulfilmentInput.OrderCancelled) {
            return completed(state.withStep(Step.CANCELLED), Step.CANCELLED, "order-cancelled", "ORDER_CANCELLED");
        }
        return ignore(state, in, context);
    }
    ...
    private static ProcessDecision<OrderFulfilmentState> ignore(
        ...
        if (in instanceof OrderFulfilmentInput.OrderPlaced) {
            throw new IllegalStateException(
                    "OrderPlaced is a start-only input and must not reach react for order " + state.orderId());
        }
        ProcessLifecycle current = context.currentLifecycle().orElseThrow(
                () -> new IllegalStateException("react requires a current lifecycle for order " + state.orderId()));

...
/**
 * Invariant: the same SKU must not appear on more than one line — quantities should be consolidated
 * onto a single line instead. This spans all lines, so it is an aggregate-level invariant on the
 * {@link Order} root, not a per-line check.
 */

...
/**
 * A <em>pure</em> domain policy that decides whether a cancellation is permitted, given only
 * facts the order already owns (its id, its customer, its status) and the evidence carried by
 * the {@link CancellationReason}. It is the rich arbiter for the one transition whose legality
 * depends on <em>why</em> and on <em>proof</em> — everything a flat transition table cannot express.
 *
...
public final class OrderLifecyclePolicy {
    ...
    public void ensureCancellable(
        ...
        if (status == OrderStatus.SHIPPED) {
            throw new DomainException(RETURN_REQUIRED, "a shipped order must enter the return flow, not be cancelled");
        }
    ...
    private void ensureInventoryCancellationAllowed(
        ...
        if (status != OrderStatus.FULFILMENT_IN_PROGRESS) {
            throw new DomainException(
                    INVENTORY_FAILURE_NOT_APPLICABLE, "an inventory failure only cancels an order under fulfilment");
        }
        if (!failure.failure().belongsTo(orderId)) {
            throw new DomainException(
                    RESERVATION_FAILURE_ORDER_MISMATCH, "the reservation failure does not belong to this order");
        }
    }

    private void ensurePaymentCancellationAllowed(
            OrderId orderId, OrderStatus status, CancellationReason.PaymentDeclinedAfterStockReleased failure) {

        if (status != OrderStatus.FULFILMENT_IN_PROGRESS) {
            throw new DomainException(
                    PAYMENT_FAILURE_NOT_APPLICABLE, "a payment failure only cancels an order under fulfilment");
        }
        // Both pieces of compensation evidence must pertain to the very order being cancelled.
        if (!failure.paymentDecline().belongsTo(orderId) || !failure.stockRelease().belongsTo(orderId)) {
            throw new DomainException(
                    COMPENSATION_EVIDENCE_ORDER_MISMATCH, "the compensation evidence does not belong to this order");
        }
    }
    ...
    private void ensureReviewCancellationAllowed(
        ...
        if (status != OrderStatus.AWAITING_REVIEW) {
            throw new DomainException(ORDER_NOT_AWAITING_REVIEW, "only an order awaiting review can be review-rejected");
        }

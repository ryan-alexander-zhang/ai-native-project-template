...
public enum OrderingErrorCode implements ErrorCode {
    ...
    CUSTOMER_CANCELLATION_WINDOW_CLOSED("ordering.customer-cancellation-window-closed", ErrorCategory.CONFLICT),
    /** An inventory-failure cancellation was attempted on an order that is not under fulfilment. */
    INVENTORY_FAILURE_NOT_APPLICABLE("ordering.inventory-failure-not-applicable", ErrorCategory.CONFLICT),
    /** The supplied reservation-failure evidence belongs to a different order. */
    RESERVATION_FAILURE_ORDER_MISMATCH("ordering.reservation-failure-order-mismatch", ErrorCategory.DOMAIN_RULE),
    ...
    COMPENSATION_EVIDENCE_ORDER_MISMATCH("ordering.compensation-evidence-order-mismatch", ErrorCategory.DOMAIN_RULE),
    ...
    REVIEW_DECISION_ORDER_MISMATCH("ordering.review-decision-order-mismatch", ErrorCategory.DOMAIN_RULE),

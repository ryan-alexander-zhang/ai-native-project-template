...
/**
 * The order-fulfilment business port: the stable entry the ordering context drives the flow through,
 * named by the order id. It hides the durable runtime behind intention-revealing methods, so the
 * inbound adapter and the domain-event bridge depend on this — not on the process-manager runtime or
 * on how the instance is addressed. Swapping the provider (native / Temporal / Seata) replaces the
 * implementation without touching these call sites (design-00004 §13.1).
 */

 ...
 * one available answer is not one.
 *
 ...
 * ReviewDecisionRef.Rejection} type). The sealed {@code CancellationReason} forced the domain to
 * handle the branch; nothing forced the application to offer it. No result.
 */
@OperationLog(
...
public record RejectReview(@NotBlank String orderId) implements Command<Void> {}

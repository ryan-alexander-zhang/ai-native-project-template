 ...
 *
 * <p>Domain-event subscribers belong here, in the application layer, not in an inbound adapter:
 * an adapter translates an external transport into a command and must not reach into the context's
 * own domain types. Keeping this subscription in the application layer is why
 * {@code ordering-adapter} needs no dependency on {@code ordering-domain}. The events are published
 * in-process, synchronously, within the transaction that recorded them.
 */
@Component
@DomainEventHandler

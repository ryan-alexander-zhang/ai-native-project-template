 ...
 * inventory and payment, and the ordering context's own terminal facts. A sealed set so the
 * {@link OrderFulfilmentDefinition} handles every case exhaustively. Each input carries only business
 * fields (its correlation is the order id); transport metadata travels in the {@code CommandContext}.
 */
public sealed interface OrderFulfilmentInput extends ProcessInput {
    ...
    record StockReservationFailed(String orderId, String code, String reason) implements OrderFulfilmentInput {
    }
    ...
    record PaymentDeclined(String orderId, String code, String reason) implements OrderFulfilmentInput {
    }

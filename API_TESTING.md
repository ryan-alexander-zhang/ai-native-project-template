...
Every HTTP API ships an OpenAPI 3 spec generated by the framework from the
code — never hand-written. A test fetches the spec and writes it to
`api-tests/openapi.json`; the run fails when the committed file differs, so a
contract change shows in the PR diff. Exposure is governed by
...
branches, and (for commands) idempotent replay semantics. `api-tests/openapi.json`
is current.

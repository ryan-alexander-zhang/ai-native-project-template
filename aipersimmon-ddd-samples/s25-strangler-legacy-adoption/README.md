...
① 版本列 `DEFAULT 0` 让每一行历史数据都"看起来没保存过",报错却只说"并发创建 / 工厂忘了
restoreVersion"——现在信息里加了第三个原因,类 javadoc 也写明"为既有表加版本列时默认值必须是 1";
② 自增主键的 INSERT **会通过**(守卫只在 update 路径),错误归属已落库且已流到下游才有人报错——
现在 insert 路径也有前置检查,信息点名 `BIGSERIAL` / `AUTO_INCREMENT` 并给出取号办法。
本篇两条相关用例因此改向:第一条断言"当场被拒且表里一行都没有",第二条改为直接走 update 路径,
因为那两条守卫失败的原因本来就不同(一个会匹配全表,一个会静默成功)。

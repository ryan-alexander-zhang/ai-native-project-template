...
只活在界面里、不进 `docs/` 的状态：当前选中、下钻所在文档、检视面板的展开行、详情面板的目标、全局覆盖率视图的展开行、当前呈现的会话与各会话的终端滚动位置。刷新后按 id 保持。
...
Agent 会话（四种皆然，`spec-00003` 前仅推进）启动时记录的 `docs/` 已脏路径及其内容摘要；会话结束时以内容差集确定该次 commit 的暂存集。
_Avoid_：基线、备份
...
种类 commit、刷新）照常走一次；不是评审动作，不删除澄清状态文件。服务端

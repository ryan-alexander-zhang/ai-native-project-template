...
只活在界面里、不进 `docs/` 的状态：当前选中、下钻所在文档、检视面板的展开行、详情面板的目标、全局覆盖率视图的展开行、当前呈现的会话与各会话的终端滚动位置、编辑器的问题列表视图态及其定位线程。刷新后按 id 保持。
...
Agent 会话（终端形态四种皆然，`spec-00003` 前仅推进、共写自 `spec-00006` 起；答疑不作快照，`spec-00005-FR-4`）启动时记录的 `docs/` 已脏路径及其内容摘要；会话结束时以内容差集确定该次 commit 的暂存集。
_Avoid_：基线、备份
...
**问题列表（Ask List）**：
一个文档积累的全部答疑线程，作为其编辑器的第三视图态（与编辑、预览
并列）逐项呈问题与状态、点开呈现该项问答，可浏览。
...
（编辑器缓冲随 agent 写入重载，非等待期只读），结束时经收束过滤一次
commit——仅目标文档与合式新建 `reference` 落地；不是评审动作，不改变
文档状态，由 `rule-00001-BR-28` … `BR-30` 与 `spec-00006` 持有。

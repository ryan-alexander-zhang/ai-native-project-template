...
Agent 会话（终端形态三种皆然，`spec-00003` 前仅推进；答疑不作快照，`spec-00005-FR-4`）启动时记录的 `docs/` 已脏路径及其内容摘要；会话结束时以内容差集确定该次 commit 的暂存集。
_Avoid_：基线、备份
...
种类 commit——答疑无 commit 收尾、刷新）照常走一次；不是评审动作，不删除澄清状态文件。服务端
...
白板通过本地 CLI（Claude Code / Codex 等）发起的一次代理过程——推进的文档代写、澄清的逐题提问、审计的对照 README 审查（三者在内嵌终端中实时交互），或答疑的 headless 调用（无终端）；写权限按流程配置约束（MVP 默认仅 `docs/`，答疑只读），浏览器断开后会话在服务端存续。
_Avoid_：AI 任务、机器人

...
- **front matter 的偏移换算是双向的，前缀长度只算一处。** 存在**两个
  ...
  锚的坐标系是整文件的**规范化文本**（行尾 `\r\n`/裸 `\r` 统一为
  `\n`，design-00001 §12.2——前端在切片、区间安装与高亮前对缓冲做
  同一次规范化，本仓当下无 CRLF 文件、系恒等变换，写死是为将来两侧
  不各说各话；CodeMirror 的缓冲也含 front matter），而预览
...
- **定位不限未提交**（`AC-9.6`/`AC-9.12` 说的是「一条锚可定位的标注」，
  ...
  **清除时机**（临时的意思要写死）：下一次定位、目标行以外的一次点击、
...
- **终止 / 失败回到未提交的批**（`spec-00007-FR-10`）：徽标回到未提交，
  其下留一行 `muted` 的「上一次共写已终止 / 已失败」直到该条被修改或
  再次提交——拒绝不吞标注，但也不假装什么都没发生过。

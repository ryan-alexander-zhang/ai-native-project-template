...
type: design
status: active
informs: [spec-00001-docs-whiteboard, spec-00002-whiteboard-governance, spec-00003-whiteboard-parallel-sessions, spec-00004-whiteboard-desktop-notifications, spec-00005-whiteboard-ask-threads]
---
...
- **双入口，一个输入**：浮窗工具栏的答疑项**保留位置、换行为**——点击
  ...
  形），开同一个输入。空问题禁用提交（服务端 422 兜底）。提交即收起
  （`spec-00005-FR-3`），不打开终端面板。异常文档：
...
- **编辑器第三视图态（spec-00005-FR-9）**：编辑器头部的 Source/Preview
  ...
  `AC-7.1` 的界面半边），失败/终止的问带重发按钮；线程带「接续已失效」
...
- **会话面板（§12 扩展）**：行字段照旧（kind=ask 自明形态）；**每个
  ...
  `DELETE /api/sessions/:id`、二次确认口径同终端头部入口。ask 行点击

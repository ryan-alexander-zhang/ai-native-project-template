...
(`aipersimmon/db/migration/{component}/{vendor}`;outbox 放 `aipersimmon-ddd-outbox` core、inbox 放
新增的 `aipersimmon-ddd-inbox`,jdbc 与 mybatis-plus 适配器共享),可选的共享 `aipersimmon-ddd-flyway`
starter 用每组件独立历史表自动应用,或复制进消费者自己的 Flyway/Liquibase。

...
[decision-00006-integration-event-transport-selection](decision-00006-integration-event-transport-selection.md)(三种传输、outbox 底座)与
[decision-00013-command-context-and-causation-propagation](decision-00013-command-context-and-causation-propagation.md)(envelope 承载 correlation/causation),
...
`docs/reference` 语料对"事件信封元数据"支撑很弱(见 [decision-00013-command-context-and-causation-propagation](decision-00013-command-context-and-causation-propagation.md)
...
2. **`IntegrationEvent` 声明契约身份**:`eventType()` / `eventVersion()`(逻辑类型 + schema 版本)与 `subject()`
   ...
   [issue-00005-integration-event-logical-type-resolution](../issue/issue-00005-integration-event-logical-type-resolution.md))**:本决策初稿曾定 `eventType()` 默认简单类名、
   ...
   取自注解。这是唯一新增的注解,承 [decision-00009-event-type-markers-and-handler-contracts](decision-00009-event-type-markers-and-handler-contracts.md) 的克制(不加
...
3. **消费端类型解析可插拔**:`IntegrationEventCatalog`(framework-free 接口,`Optional<Class> lookup(type, version)`)
   ...
   **扫描应用包下的 `IntegrationEvent` 实现**,按各类 `(@EventType name, version)` 建表(与该类被发布时写线的
   ...
   [issue-00003-messaging-delivery-reliability](../issue/issue-00003-messaging-delivery-reliability.md) 落地)。`InProcessOutboxDispatcher` 与
   ...
   [issue-00005-integration-event-logical-type-resolution](../issue/issue-00005-integration-event-logical-type-resolution.md) 修订为上述 `@EventType` 必填 + `(type, version)` 键 +
   无回退。)
...
- **命题一 —— 类型必须是逻辑契约,不是 Java 类。** 发布语言(published language)的类型标识应稳定、语言中立;FQCN
   ...
   承接 [decision-00006-integration-event-transport-selection](decision-00006-integration-event-transport-selection.md))。
...
- **supersede** [decision-00006-integration-event-transport-selection](decision-00006-integration-event-transport-selection.md) 中"按 `type` 反序列化"的隐含 FQCN 语义:
...
- **`@EventType` 必填(按 [issue-00005-integration-event-logical-type-resolution](../issue/issue-00005-integration-event-logical-type-resolution.md) 修订)**:每个 `IntegrationEvent`
...
- [decision-00006-integration-event-transport-selection](decision-00006-integration-event-transport-selection.md)、[decision-00013-command-context-and-causation-propagation](decision-00013-command-context-and-causation-propagation.md)、[decision-00009-event-type-markers-and-handler-contracts](decision-00009-event-type-markers-and-handler-contracts.md)、[analysis-00002-domain-vs-integration-events](../analysis/analysis-00002-domain-vs-integration-events.md)。

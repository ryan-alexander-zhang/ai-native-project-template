...
两种入口如何归一、事务与幂等语义、以及模板/隐私/租户的安全边界。承接预研 [analysis-00013-operation-log-component](../analysis/analysis-00013-operation-log-component.md)
（逐调用链审计了 `mzt-biz-log` / `log-record` 两套参考实现）与结构设计 [design-00008-operation-log-component](../design/design-00008-operation-log-component.md)
（本 ADR 只固化"决策与取舍"，机制细节以 design-00008 为准，不在此重复）。

本 ADR 受既有决策约束：[decision-00021-command-handler-reuse-and-cross-aggregate-placement](decision-00021-command-handler-reuse-and-cross-aggregate-placement.md)、
...
[decision-00016-durable-runtime-staged-message-identity](decision-00016-durable-runtime-staged-message-identity.md)、[design-00003-exception-model](../design/design-00003-exception-model.md)、[design-00005-observability-and-distributed-tracing](../design/design-00005-observability-and-distributed-tracing.md)。
...
5. **（D3）注解只标 application `Command`、由 `CommandInterceptor` 解释；不做通用 method AOP。** `@OperationLog`
   是叠加元数据，不替代 `Command<R>`（对齐 [decision-00011-cqrs-write-contracts-as-interfaces-not-annotations](decision-00011-cqrs-write-contracts-as-interfaces-not-annotations.md)）；
...
7. **（D4/D12）启动期 fail-fast**：一个 input type 出现"注解+Definition 双绑、重复 Definition、泛型不可判定"即启动失败；
   ...
   部分失败语义，另开 ADR。
8. **（D4/D11）无 ambient 状态**（对齐 [decision-00012-no-ambient-per-command-state](decision-00012-no-ambient-per-command-state.md)）：成功与失败两路各持
...
9. **（D5）两个 interceptor + 可插拔 `FailureClassifier`。** `CompletedOperationLogInterceptor` 在事务内、handler 外层
   ...
   [design-00003-exception-model](../design/design-00003-exception-model.md)（预期业务/校验/授权异常 → `REJECTED`；`ConcurrencyConflictException` → `FAILED` +
...
12. **（D9）时间有序 `recordId` + result-kind 感知的 `idempotencyKey`。** `recordId` 用 UUIDv7/ULID（经注入的
    ...
    （[decision-00016-durable-runtime-staged-message-identity](decision-00016-durable-runtime-staged-message-identity.md) 保证 `messageId` 跨重投稳定）"先 FAILED 后 SUCCEEDED"产生
    两行、各自幂等（见命题五）。direct-API 若可重试须由调用者传稳定 key。
13. **（D13）v1 只同步本地 append。** 不复用 Integration Event outbox（[decision-00014-cloudevents-integration-event-contract](decision-00014-cloudevents-integration-event-contract.md)）、
...
17. **（D11）actor/tenant 用无状态 resolver、可信来源，不扩展 `CommandContext`。** resolver 无状态/无 IO/无副作用；
    ...
    （对齐 [decision-00013-command-context-and-causation-propagation](decision-00013-command-context-and-causation-propagation.md)）；跨异步边界传播"原始操作者"另立 ADR。
...
见记忆 `process-manager-engine`——注意 [design-00004-durable-process-manager-runtime](../design/design-00004-durable-process-manager-runtime.md) 原文描述的是更早的三模块形态，
...
符合 [decision-00021-command-handler-reuse-and-cross-aggregate-placement](decision-00021-command-handler-reuse-and-cross-aggregate-placement.md) 与 [decision-00011-cqrs-write-contracts-as-interfaces-not-annotations](decision-00011-cqrs-write-contracts-as-interfaces-not-annotations.md)。
...
durable 命令运行时是至少一次投递（[decision-00016-durable-runtime-staged-message-identity](decision-00016-durable-runtime-staged-message-identity.md)）。若幂等键只含
...
- 后续待办：本 ADR 已产出 [spec-00001-operation-log-component](../spec/spec-00001-operation-log-component.md) 与 [plan-00010-operation-log-implementation](../plan/plan-00010-operation-log-implementation.md)（均 `draft`）；
...
- [analysis-00013-operation-log-component](../analysis/analysis-00013-operation-log-component.md)（两套参考实现的逐调用链审计与需求追踪）
- [design-00008-operation-log-component](../design/design-00008-operation-log-component.md)（本 ADR 的机制承载文档：模型、端口、DDL、interceptor、时序）
- [spec-00001-operation-log-component](../spec/spec-00001-operation-log-component.md)（feature spec）、[plan-00010-operation-log-implementation](../plan/plan-00010-operation-log-implementation.md)（落地计划）
- [decision-00021-command-handler-reuse-and-cross-aggregate-placement](decision-00021-command-handler-reuse-and-cross-aggregate-placement.md)、[decision-00011-cqrs-write-contracts-as-interfaces-not-annotations](decision-00011-cqrs-write-contracts-as-interfaces-not-annotations.md)、
  [decision-00012-no-ambient-per-command-state](decision-00012-no-ambient-per-command-state.md)、[decision-00013-command-context-and-causation-propagation](decision-00013-command-context-and-causation-propagation.md)、
  [decision-00014-cloudevents-integration-event-contract](decision-00014-cloudevents-integration-event-contract.md)、[decision-00016-durable-runtime-staged-message-identity](decision-00016-durable-runtime-staged-message-identity.md)
- [design-00003-exception-model](../design/design-00003-exception-model.md)（`FailureClassifier` 默认分类对齐）、[design-00004-durable-process-manager-runtime](../design/design-00004-durable-process-manager-runtime.md)
  （core/engine/后端 四段式模板）、[design-00005-observability-and-distributed-tracing](../design/design-00005-observability-and-distributed-tracing.md)

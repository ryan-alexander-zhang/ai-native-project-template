...
[analysis-00010-exception-model](../analysis/analysis-00010-exception-model.md)(及其引用的 [analysis-00008-web-api-response-envelope](../analysis/analysis-00008-web-api-response-envelope.md));
落地模块设计见 [design-00003-exception-model](../design/design-00003-exception-model.md)。本文只记**决策与理由、被拒选项**。

## Context

现状是两极分化(详见 [analysis-00010-exception-model](../analysis/analysis-00010-exception-model.md)):传输层(`-web`/`-web-spring`)的 RFC 9457 线上
...
[design-00002-web-layer](../design/design-00002-web-layer.md) §八 的旗舰示例产不出来。

约束不可动:[analysis-00006-ddd-building-blocks-library](../analysis/analysis-00006-ddd-building-blocks-library.md) 的**依赖一律指向内/下**、**纯净层
...
- `-web` 用**组合而非继承**接通:领域抛出的纯 `ErrorCode` 由 `ProblemRegistry.resolve(ErrorCode)` 在边界解析成
  ...
  1:1、契约随领域码膨胀,且六个参考项目无一如此),理由详见 [design-00003-exception-model](../design/design-00003-exception-model.md) §4.7。

### 二、`Invariant` 一等抽象 + `AbstractAggregateRoot.checkInvariant()`

- 库**提供** `Invariant`(`isBroken()`/`message()`/**必填** `errorCode()`)+ 聚合根 `checkInvariant(rule)` → `InvariantViolationException`(采 modular-monolith-with-ddd 的 `IBusinessRule`/`CheckRule` 模式,但改名 `Invariant` 以避免与边缘输入校验 `Validator` 混淆——见 [design-00003-exception-model](../design/design-00003-exception-model.md) §4.6)。
- **但它是升级项、不是默认**:聚合内不变量默认用 coded `throw`,仅当**非平凡 / 可复用 / 需清点或组合**时才升级为 `Invariant`(判据见 [design-00003-exception-model](../design/design-00003-exception-model.md) §4.5)。两者走同一错误码通道,升降级不影响线上契约。避免"一行条件套个类"的仪式化。
...
采用 [design-00003-exception-model](../design/design-00003-exception-model.md) §六 映射表。核心取舍(纠正早先"`DomainException` 默认 409"的可疑默认):
...
- 401/403 → ProblemDetail 仅在 classpath 有 spring-security 时条件化激活(承 [decision-00007-web-api-response-envelope](decision-00007-web-api-response-envelope.md) §六)。

## 明确不做 / 取舍

1. **默认 throw + 结构化错误码,不上 `Result`/`Either` 作默认口径**。理由(诚实排序,见 [design-00003-exception-model](../design/design-00003-exception-model.md) §十):
...
- **`ProblemType extends ErrorCode`(身份即传输)**:见 [design-00003-exception-model](../design/design-00003-exception-model.md) §4.7,已改为组合。
- **只在边界(adapter)现拼错误码**:边界无法得知领域语义,码不稳定、易漂移,拒(码必须"从内到外")。
- **全面函数式(`Either` 进 core)**:见"明确不做 1"。

## Consequences

- **正向**:`code`/`type` 对领域异常首次可达,[design-00002-web-layer](../design/design-00002-web-layer.md) §八 示例可复现;规则可测可组合;
  未找到/并发/校验有稳定语义。纯/脏与依赖向内不变。
- **治理成本**:`code`/`typeUri` 一旦发布即**对外契约**,变更走版本;错误码枚举跨 BC 增长需命名前缀治理;
  `-application` 新增两个语义子类,消费者需知其映射。
- **迁移**:全为加法(旧构造保留),存量不强制一次性改;脚手架作为示范应改齐(见 [design-00003-exception-model](../design/design-00003-exception-model.md) §十二)。

## Sources

- [analysis-00010-exception-model](../analysis/analysis-00010-exception-model.md) —— 缺口证据、参考项目与大厂对照、验收清单。
- [analysis-00008-web-api-response-envelope](../analysis/analysis-00008-web-api-response-envelope.md) —— 传输层 RFC 9457 线上契约的一手证据(本决策沿用,不改格式)。
- [analysis-00006-ddd-building-blocks-library](../analysis/analysis-00006-ddd-building-blocks-library.md) —— framework-free / 依赖向内铁律。

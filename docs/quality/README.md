...
- the obligations table, right after the profile: one row per Drives entry of
  every chosen value, columns dimension, value, owed, state. State is the id of
  the `QR`, `QS`, or `decision` that pays it, `open`, or
  `waived: <decision id>`. `open` is allowed while the doc is `active` — the
  `decision`s are written in the `architecture` stage; `QUALITY.md` Definition
  of Done gates when each must close

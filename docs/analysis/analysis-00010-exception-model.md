...
- **per-BC 错误码目录**的设计意图(`decision-00007` 问题5)——最终落地为 `ErrorCode`(身份)+ `ProblemCatalog` override + category family 的组合(§4.7),非 `ProblemType extends ErrorCode`。
...
| **显式 `Invariant` 抽象 + `checkInvariant()`** | modular-monolith-with-ddd(`IBusinessRule`/`BusinessRuleValidationException`,聚合根 `CheckRule(rule)`) | ❌ 规则散落成 `if (...) throw new DomainException("...")` |
...
**参考项目共识里、本库最该补的三样**:①`Invariant` 为一等对象;②稳定机器可读错误码/typed rejection;
...
1. **贯穿式错误码**:`-core` 里 framework-free 的 `ErrorCode` 抽象;`DomainException`/`ApplicationException`
   可携带;`-web` 的 `ProblemRegistry` 在边界把 `ErrorCode` 解析成 `ProblemDescriptor`(override 或 category family,**组合非继承**,§4.7);advice 从抛出的异常读出 `code` 填进 ProblemDetail。→ 解 #1、#2、#7。
2. **`Invariant` 一等抽象 + `AbstractAggregateRoot.checkInvariant()`**,供**够格**不变量(非平凡/可复用/需清点组合)使用,琐碎守卫仍 coded `throw`(判据见 design §4.5)。→ 解 #5。
3. **完整且正确的异常→HTTP 映射**,含 `ConstraintViolationException`;统一两处 Bean Validation 的 `FieldError`。→ 解 #3、#8。
4. **随库交付 i18n bundle** + 让 filter 路径也走 MessageSource;脚手架给真实 `ProblemCatalog` override + category family 样例。→ 解 #2、#4。

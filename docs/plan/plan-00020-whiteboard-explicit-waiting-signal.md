...
- [design-00001-docs-whiteboard](../design/design-00001-docs-whiteboard.md)
  —— 会话通道一节增补：信号识别（`ESC]777;notify;` 前缀匹配、跨输出块
  缓冲及其上界）、锁存状态机（置位/幂等/解除三事件）、与静默判定的定序
  （信号识别先于输出解除）（待 T1 修订轮）。
...
- **T1 — design-00001 修订轮**：板上转 `draft` → 会话通道一节增补信号
  识别与锁存状态机（见 Design）→ 审计 → 接收。
...
- **T4 — spec-00004 括注校正轮**：FR-2 括注的翻摆前提原地校正为「信号
  ...
  语义与 AC 均不动——纯括注校正，走最小修订轮。

## Detailed Acceptance Path

1. T1 文档轮完成 → verify: design-00001 重新 `active` 且会话通道一节
   含信号识别与锁存状态机。

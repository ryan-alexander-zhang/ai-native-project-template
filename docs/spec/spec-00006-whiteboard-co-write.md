...
- **spec-00006-FR-4** (State-driven) 共写会话运行期间，系统应将目标文档的
  ...
  **编辑与保存**为只读禁用——只读只作用于 Source 视图：预览与问题列表
  ...
  design-00002 §14/§15，此后切换照常保持）。缓冲重载例外**仅及目标
...
- **spec-00006-FR-5** (Complex) 当共写会话等待输入、且用户在编辑器修改并
  ...
  保存仍按既有口径拒绝，同为保护），并在用户下一次向 agent 输入**可打印
  内容**（控制字节、CSI/OSC 序列与裸回车不算——注记顺延、不随其注入也
  不消耗，细则见 design-00001 §11.4）时附带「用户已手改目标文档、动笔前
  须重读」的注记；用户保存后未再输入即结束会话的，注记随会话消亡（手改
...
- **spec-00006-FR-6** (Unwanted) 如果共写会话在 `docs/` 下产生了目标文档
  ...
  会话的快照后差集路径，与用户动作已写盘而未成 commit 的路径
  （`spec-00001-FR-20` 的保留物）——二者不动、不暂存（并行产出与用户
  保存不因共写收束被毁）。**合式**指：front matter 可解析、type 为
  `reference`、status 为 `draft`、位于规范路径 `docs/reference/<id>.md`
  且文件名与 id 一致、id 形态合法且本会话全部新建作为**集合**自收束时点
  既有最大编号**加一**起连续取号、编号两两不同（「既有」含运行中会话的
  保留号；集合读法与撞 id 判定细则见 design-00001 §11.3）。目标文档
...
- **spec-00006-AC-6.5** (spec-00006-FR-6)
  Given 共写会话收束时，另一份文档的路径落在一个仍在运行的推进会话的
  快照后差集里
  When 收束过滤运行

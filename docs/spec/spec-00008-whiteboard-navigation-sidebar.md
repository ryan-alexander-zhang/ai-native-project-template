...
| S3 | 作为文档负责人，我要能收起不关心的类型组、收起整条导航栏，而且下次打开还是我上次的样子 | spec-00008-FR-4, spec-00008-FR-5 |
| S4 | 作为文档负责人，文档增删改后列表要跟着变，我折叠的组不要被打开 | spec-00008-FR-6 |
...
- **spec-00008-FR-4** (Event) 当用户点击一个组头时，系统应折叠或展开该组
  （折叠后组头与计数仍在、行不呈现）；各组折叠态按类型持久于浏览器本地
  （不入 `docs/`），缺省全部展开。用户折叠的是当前选中行所在的组时，该组保持
  折叠——FR-3 的展开只随选中**变化**发生，不随折叠动作反弹（域主 2026-09-02 裁定）。
...
- **spec-00008-FR-6** (Event) 当刷新到达时，系统应按新图重建导航栏：新增
  ...
  状态、标题的变化即时反映；各组折叠态与选中行高亮**按 id 保持**，不因重建
  而重置（就近关闭：消失的是选中文档时，高亮随选中一并消失）。
...
- **spec-00008-AC-1.4** (spec-00008-FR-1)
  Given 一份文档的 front matter 不合法（异常节点）
  When 白板呈现
...
- **spec-00008-AC-6.2** (spec-00008-FR-6)
  Given `plan` 组已折叠且 `spec-00001` 已选中并高亮
  When 一次不涉及这两者的刷新到达
  Then `plan` 组仍折叠，`spec-00001` 的行仍高亮

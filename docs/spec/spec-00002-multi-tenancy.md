...
技术设计在 [design-00009-multi-tenancy-tenant-id](../design/design-00009-multi-tenancy-tenant-id.md)（本 spec 不内联设计）；决策边界见
[decision-00018-multi-tenancy-boundaries](../decision/decision-00018-multi-tenancy-boundaries.md)。术语见 `CONTEXT.md` 的 Multi-Tenancy Language 区。
...
silo **seam** 预留在范围内（FR-17），其路由实现不在。
...
- 受约束于 [decision-00012-no-ambient-per-command-state](../decision/decision-00012-no-ambient-per-command-state.md)（`TenantContext` 是 trusted-boundary 一次写入的不可变身份，
  非可变每命令状态——见 FR-3）、[decision-00013-command-context-and-causation-propagation](../decision/decision-00013-command-context-and-causation-propagation.md)（写侧权威载体为
  `CommandContext.tenantId`）、[decision-00014-cloudevents-integration-event-contract](../decision/decision-00014-cloudevents-integration-event-contract.md)（跨进程走扩展属性 + `ce_tenantid`）、
  [decision-00016-durable-runtime-staged-message-identity](../decision/decision-00016-durable-runtime-staged-message-identity.md)（at-least-once 幂等不因租户改变）、
  [design-00005-observability-and-distributed-tracing](../design/design-00005-observability-and-distributed-tracing.md)（trace 与租户分离传播）。
- **跨组件一致性**：全仓库共用**单一哨兵 `__root__`**。operation-log 链（analysis-00013 / decision-00017 /
  design-00008 / spec-00001）原用 `GLOBAL`，已统一改为 `__root__`（本 spec FR-11 保留此约束以防回归）。

## 2. Stories

多租户是框架横切能力，没有可独立发布的 story 切片；下列 story 只说明受益方，需求统一编号
`spec-00002-FR-<i>`。Story 面向**消费方开发者**与**平台运维**。
...
- **spec-00002-FR-7** (Optional) 在 MyBatis-Plus 后端启用时，系统应以原生 `TenantLineInnerInterceptor` 自动注入租户谓词与 INSERT 列，
  并 `ignoreTable` 排除 `shedlock` 与消息管道表。
- **spec-00002-FR-8** (Optional) 在 PostgreSQL 后端启用时，系统应对**请求作用域表**（领域表/读模型/`process_instance` 请求侧检索）启用 RLS；
  H2/MySQL 无 RLS，退回手工 `TenantPredicate`。
- **spec-00002-FR-9** (Optional) 在 PostgreSQL RLS 后端启用时，请求作用域事务应在事务内 `SET LOCAL app.tenant = <resolved>`；
...
- **spec-00002-FR-11** (Optional) 在多租户开启时，当请求解析不出租户时，`MissingTenantPolicy` 默认 `REJECT`（400/401）；
  全仓库共用单一哨兵 `__root__`（operation-log 链已对齐）。
- **spec-00002-FR-12** (Optional) 在多租户开启时，当**基础设施**要盖章或过滤 `tenant_id` 而当前线程无绑定时，
  系统应抛 `MissingTenantException`，**不得**回退哨兵。该决策应收口在 `TenantContext.effective()` 单点，
  调用点不得各自决定回退值。多租户关闭时同一入口回退 `__root__`（N=1）。
- **spec-00002-FR-13** (Optional) 在多租户开启时，系统应为自动装配的 executor 提供租户传播（`TaskDecorator`），
  并在消费方已自带 `TaskDecorator` 时让位（Spring Boot 仅在唯一 bean 时应用，注册第二个会静默废掉对方）。
  提交时无绑定不得凭空造租户。
- **spec-00002-FR-14** (Optional) 在多租户开启时，系统**不应**把客户端可控的请求头作为默认租户来源：
  无自定义 `TenantResolver` 且未显式 `trust-header=true` 时应拒绝启动，并在错误中给出安全接法。
- **spec-00002-FR-15** (Optional) 在多租户开启时，消费的集成事件缺 `ce_tenantid` 时应按格式错误永久失败（死信），
  不得归属哨兵；多租户关闭时应容忍缺失以兼容前租户时代的消息。
- **spec-00002-FR-16** (Ubiquitous) `exclude-paths` 应按容器派发路径匹配，使路径穿越无法借用排除前缀
  跳过租户解析。
- **spec-00002-FR-17** (Optional) 作为 silo seam 预留，系统应把请求作用域数据源暴露为**单一可替换 bean**（读 `TenantContext`），
...
默认外置：机制、12 表 schema、传播链、模块清单见 [design-00009-multi-tenancy-tenant-id](../design/design-00009-multi-tenancy-tenant-id.md)。下列为 spec 级、
需在本 spec 阶段定死的**运维细节**（decision-00018 交由本 spec 裁定的三项）：
...
`header`（默认 `X-Tenant-Id`）、`trust-header`（默认 false，见 FR-14）、`exclude-paths`（默认 `/actuator/**`）、
`mybatis-plus.tenant-column`、`mybatis-plus.tenant-tables`。
自定义解析来源（subdomain / jwt-claim）由消费方提供 `TenantResolver` bean 实现，不再是配置枚举。

## 5. Links
- Decision: [decision-00018-multi-tenancy-boundaries](../decision/decision-00018-multi-tenancy-boundaries.md)
- Design: [design-00009-multi-tenancy-tenant-id](../design/design-00009-multi-tenancy-tenant-id.md)
- Plan: [plan-00011-multi-tenancy-implementation](../plan/plan-00011-multi-tenancy-implementation.md)
- 术语：`CONTEXT.md` → Multi-Tenancy Language
- 已对齐：[spec-00001-operation-log-component](spec-00001-operation-log-component.md) 等 operation-log 文档哨兵 `GLOBAL` → `__root__`

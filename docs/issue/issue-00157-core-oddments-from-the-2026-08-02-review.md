...
- [ ] **嵌套 dispatch 破坏"一命令一事务"契约**：`TransactionTemplateUnitOfWork.java:22`
  ...
  语义；或让嵌套传播成为显式选择。
- [ ] **并发 create 冲突逃出并发词汇表**：insert 路径（`JdbcAggregateRepository.java:59-60`、
...
- [ ] **handler 注册的启动期严格性弱于 precheck**：`PrecheckCommandInterceptor.commandTypeOf`
...
- [ ] **rehydration 忘 `restoreVersion` 的失败症状离故障点太远**：version-0 聚合走 insert
  分支、死于与真实错误无关的重复键。在重复键 + version==0 的路径上给出指名
  `restoreVersion` 的报错提示。
- [ ] **`JdbcAggregateRepository.insert` 返回值声明了但没人看**（契约 :100-104 vs
  `saveAggregate` :59-60）：子类用 `INSERT ... ON CONFLICT DO NOTHING` 会静默"保存"零行
  并照常发事件。检查返回值，0 行时抛。
- [ ] **值对象不可变规则的 javadoc 措辞过强**：`BuildingBlockRules.java:99-110` 只查
...
- [ ] **`versionAdvanced()` 的防线依赖消费方采纳 ArchUnit**：运行时锁只对跑了

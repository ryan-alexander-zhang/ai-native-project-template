 ...
 * exponential backoff: its {@code next_attempt_at} is pushed out (see {@link RetryBackoff}) so the
 * poll skips it until then, rather than re-attempting it every second. When a transient failure has
 * burned through {@code max-attempts} it too is dead-lettered. In every give-up case the row is
...
public class OutboxRelay {
   ...
   * the row with no {@code next_attempt_at} (which would have the poll re-select and re-dispatch it
   * every second). Instead it backs the row off — pushing {@code next_attempt_at} without counting
   * an attempt, so the row stays selectable — and the move is retried at the backoff cadence,
   * self-healing once the store recovers.
   *
  ...
  /**
   * A null or blank subject carries no per-aggregate ordering key (matching the query and the Kafka
   * partition key), so it never blocks or is blocked.
   */

 ...
 */
public class MybatisPlusWebStoreCleanup {

  private static final Logger log = LoggerFactory.getLogger(MybatisPlusWebStoreCleanup.class);
  ...
  public void sweep() {
    ...
    int idempotency =
        idempotencyMapper.delete(
            new LambdaQueryWrapper<IdempotencyRecord>().le(IdempotencyRecord::getExpiresAt, now));
    int nonces =
        nonceMapper.delete(
            new LambdaQueryWrapper<NonceRecord>().le(NonceRecord::getExpiresAt, now));
   ...
   * MybatisPlusRateLimiter} sweeps its own bucket on every call — so what is left here is the cold
   * ones, the keys never seen again.
   */
  private int purgeRateLimitWindows(Instant now) {
    return rateLimitMapper.delete(
        new LambdaQueryWrapper<RateLimitRecord>()
            .lt(RateLimitRecord::getWindowStart, now.minus(rateLimitRetention)));
  }
}

...
/**
 * The signed material a {@link RequestSignatureVerifier} checks: the presented
 * signature, the body it was computed over, the request timestamp, and an optional
 * nonce. The timestamp is compared against a tolerance window by the filter; the
 * nonce, when present, is deduplicated by a {@link ReplayGuard}.
 *

 ...
 * with ERROR 1093 ("can't specify target table for update in FROM clause"). The previous test
 * suite missed this because the deadline schedule/cancel path was only covered on H2 (which does
 * not enforce 1093) and the MySQL container test exercised only effect claiming. This is that
 * missing coverage: it must run clean on MySQL and actually flip the current generation to
 * CANCELLED while leaving older generations untouched.
 */
@Testcontainers
class DeadlineCancelMysqlTest {
    ...
    private ProcessDeadlineInsert insert(String deadlineId, long generation) {
        return new ProcessDeadlineInsert(
                deadlineId, instance, name, generation, NOW.plusSeconds(600),
                "review-timeout", 1, "{}".getBytes(StandardCharsets.UTF_8),
                "corr-1", "cause-1", null, null);
    }

    private String statusOf(String deadlineId) {
        return jdbc.queryForObject(
                "SELECT status FROM aipersimmon_process_deadline WHERE deadline_id = ?",
                String.class, deadlineId);
    }
}

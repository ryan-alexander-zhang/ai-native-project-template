 ...
 * lazily on scrape; a short-lived memoized {@link BacklogSnapshot} coalesces one scrape's gauge reads
 * into a single one-pass store sample (instead of a query per gauge, and the suspended-by-source
 * query once instead of once per source). Latency and conflict-retry meters are push-based and
 * recorded by {@link MicrometerProcessObserver}.
 */
public final class ProcessManagerJdbcMeterBinder implements MeterBinder {
    ...
    public ProcessManagerJdbcMeterBinder(JdbcProcessBacklog backlog, Duration stuckThreshold, Clock clock) {
    ...
    public void bindTo(MeterRegistry registry) {
        Gauge.builder(ProcessManagerMeters.OLDEST_PENDING_EFFECT_AGE, this,
                        b -> b.sample().oldestPendingEffectAge().toMillis() / 1000.0)
                .description("Seconds the oldest due-but-undelivered effect has waited")
                .baseUnit("seconds")
                .register(registry);
        Gauge.builder(ProcessManagerMeters.OLDEST_PENDING_DEADLINE_AGE, this,
        ...
        for (String source : SUSPENSION_SOURCES) {
            Gauge.builder(ProcessManagerMeters.SUSPENDED_INSTANCES, this,

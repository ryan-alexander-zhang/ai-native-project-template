...
/**
 * A Jackson-backed {@link ProcessStateCodec} the convenience layer generates from a {@link
 * ProcessSerializationCatalog} state entry. It reads and writes exactly the one schema version
 * registered; a schema upgrade is a new explicit codec with an upcaster, never a
 * decode-then-reopen. Serialization failures surface as {@link ProcessSerializationException}.
 *
...
final class JacksonStateCodec<S> implements ProcessStateCodec<S> {
  ...
  public EncodedPayload encode(S state) {
    try {
      ...
      throw new ProcessSerializationException(
          "cannot encode state of type " + type.logicalType(), e);
    }
  }
  ...
  public S decode(EncodedPayload payload) {
    try {
      ...
      throw new ProcessSerializationException(
          "cannot decode state of type " + type.logicalType(), e);
    }
  }
}

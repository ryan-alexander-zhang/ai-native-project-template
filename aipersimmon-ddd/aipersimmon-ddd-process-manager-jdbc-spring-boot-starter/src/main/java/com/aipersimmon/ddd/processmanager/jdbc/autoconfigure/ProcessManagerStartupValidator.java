...
/**
 * Startup consistency checks beyond the four-table existence check, so a bad deployment fails fast
 * rather than at the first advance or relay. It verifies:
 *
 * <ul>
 ...
 *       cannot deliver (the "no two dispatchers per kind" half is enforced by the registry itself).
 * </ul>
 *
...
public final class ProcessManagerStartupValidator implements InitializingBean {
    ...
    for (VersionRef ref : instances.distinctVersionsInUse()) {
      try {
        ...
        throw new IllegalStateException(
            "a live instance of process '"
                + ref.processType().value()
                + "' runs under definition version "
                + ref.definitionVersion().value()
                + " but no such Definition is registered; keep the "
                + "old Definition available while instances still use it",
            missing);
      }
      try {
        ...
        throw new IllegalStateException(
            "a live instance of process '"
                + ref.processType().value()
                + "' runs under state schema version "
                + ref.stateSchemaVersion().value()
                + " but no such ProcessStateCodec is registered; keep "
                + "the old state codec available while instances still use it",
            missing);
      }
    }
  }
    ...
    for (ProcessPayloadCodec<?> codec : payloadCodecs.codecs()) {
      ...
      if (eventType == null) {
        throw new IllegalStateException(
            "integration event "
                + javaType.getName()
                + " has a payload codec but is missing @EventType");
      }
      PayloadType payloadType = codec.payloadType();
      if (!payloadType.logicalType().equals(eventType.name())
          || payloadType.version() != eventType.version()) {
        throw new IllegalStateException(
            "payload codec for integration event "
                + javaType.getName()
                + " declares logical type "
                + payloadType.logicalType()
                + "/v"
                + payloadType.version()
                + " but @EventType says "
                + eventType.name()
  ...
  private boolean hasIntegrationEventCodec() {
    return payloadCodecs.codecs().stream()
        .anyMatch(c -> IntegrationEvent.class.isAssignableFrom(c.javaType()));
  }
}

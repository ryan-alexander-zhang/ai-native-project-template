...
public final class MicrometerProcessObserver implements ProcessObserver {
    ...
    public MicrometerProcessObserver(MeterRegistry registry) {
        this.claimTimer = Timer.builder(ProcessManagerMeters.CLAIM_LATENCY)
                .description("Latency of one effect-relay claim round-trip")
                .register(registry);
        this.dispatchSuccess = Timer.builder(ProcessManagerMeters.DISPATCH_LATENCY)
                .description("Latency of one effect dispatch")
                .tag("outcome", "success")
                .register(registry);
        this.dispatchFailure = Timer.builder(ProcessManagerMeters.DISPATCH_LATENCY)
                .description("Latency of one effect dispatch")
                .tag("outcome", "failure")
                .register(registry);
        this.conflictRetries = Counter.builder(ProcessManagerMeters.ADVANCE_CONFLICT_RETRIES)

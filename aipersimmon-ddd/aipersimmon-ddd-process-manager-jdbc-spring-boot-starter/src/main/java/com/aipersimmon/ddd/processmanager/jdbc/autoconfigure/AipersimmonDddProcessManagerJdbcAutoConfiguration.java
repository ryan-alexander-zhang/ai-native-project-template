...
/**
 * Assembles the JDBC Process Manager once a {@code JdbcTemplate} is available: it collects the
 * consumer's explicitly-registered Definitions, Codecs, and Dispatchers into their registries
 * (which fail fast on a conflict), picks the SQL dialect, wires the runtime, query, relay, deadline
 * worker, and operations, and runs the workers on their own thread pools. Every bean is {@link
 * ConditionalOnMissingBean}, so a consumer overrides any of them. It scans no business packages and
 * never executes DDL.
 */
@AutoConfiguration(
    after = {
...
@ConditionalOnProperty(
    prefix = "aipersimmon.ddd.process-manager.jdbc",
    name = "enabled",
    matchIfMissing = true)
...
public class AipersimmonDddProcessManagerJdbcAutoConfiguration {
  ...
  public AipersimmonDddProcessManagerJdbcAutoConfiguration(
  ...
  public JdbcProcessDialect processDialect(
      ProcessManagerJdbcProperties properties, DataSource dataSource) {
  ...
  @ConditionalOnMissingBean
  public JdbcProcessUnitOfWork jdbcProcessUnitOfWork(
  ...
  @ConditionalOnMissingBean
  @ConditionalOnProperty(
      prefix = "aipersimmon.ddd.process-manager.jdbc.effect-relay",
      name = "enabled",
      matchIfMissing = true)
  ...
  @ConditionalOnMissingBean
  @ConditionalOnProperty(
      prefix = "aipersimmon.ddd.process-manager.jdbc.deadline-worker",
      name = "enabled",
      matchIfMissing = true)
  ...
  public ProcessWorkerScheduler processWorkerScheduler(
      ObjectProvider<JdbcProcessEffectRelay> relay,
      ObjectProvider<JdbcProcessDeadlineWorker> worker,
      ProcessManagerJdbcProperties properties) {

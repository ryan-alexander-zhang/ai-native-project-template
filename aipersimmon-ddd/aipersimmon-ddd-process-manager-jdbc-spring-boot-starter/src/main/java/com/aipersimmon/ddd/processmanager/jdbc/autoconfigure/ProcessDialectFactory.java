...
/**
 * Selects the {@link JdbcProcessDialect} from configuration or, when {@code auto}, from
 * the DataSource's product name. PostgreSQL and MySQL use {@code SKIP LOCKED}; H2 uses the
...
final class ProcessDialectFactory {
    ...
    static JdbcProcessDialect create(String configured, DataSource dataSource) {
        String id = "auto".equalsIgnoreCase(configured) ? probe(dataSource) : configured.toLowerCase(Locale.ROOT);
        return switch (id) {
            ...
            default -> throw new IllegalStateException(
                    "unsupported process-manager dialect '" + id + "'; set aipersimmon.ddd.process-manager.jdbc.dialect");
        };
    }
    ...
    private static String probe(DataSource dataSource) {
        ...
        try (Connection connection = dataSource.getConnection()) {
            ...
            throw new IllegalStateException("cannot probe the database product for dialect auto-selection", e);
        }
        ...
        throw new IllegalStateException(
                "cannot auto-select a process-manager dialect for database '" + product

 ...
 *
 * <p>Not lock-guarded: the delete is a single cutoff-bounded statement, so running it on several
 * instances at once is redundant but harmless (idempotent).
 */
public class InboxCleanup {

  private static final Logger log = LoggerFactory.getLogger(InboxCleanup.class);

  private static final String DELETE_OLD = "DELETE FROM aipersimmon_inbox WHERE processed_at < ?";

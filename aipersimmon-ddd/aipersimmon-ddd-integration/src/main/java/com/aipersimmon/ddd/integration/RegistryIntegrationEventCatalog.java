...
/**
 * Default {@link IntegrationEventCatalog}: an immutable registry mapping each known
 * {@code (type, version)} to its local class. A pair that is not registered is a miss
 * ({@link Optional#empty()}) — there is deliberately no fully-qualified-class-name
 * fallback, so an unknown message is dead-lettered rather than deserialized by
...
public final class RegistryIntegrationEventCatalog implements IntegrationEventCatalog {
    ...
    public RegistryIntegrationEventCatalog(Map<Key, Class<? extends IntegrationEvent>> byTypeAndVersion) {

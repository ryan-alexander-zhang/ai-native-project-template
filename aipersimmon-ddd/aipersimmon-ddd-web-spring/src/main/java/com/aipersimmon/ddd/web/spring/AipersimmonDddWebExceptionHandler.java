 ...
 *
 * <ul>
 *   <li>An {@link ApiException} is the first-class path — its {@link
 *       com.aipersimmon.ddd.core.error.ErrorCode} resolves through the registry to a problem
 *       descriptor (per-code override or category family).
 *   <li>A {@link DomainException} that carries an error code resolves through the registry;
 *       otherwise a business-rule violation defaults to <strong>422</strong> (well-formed but
 *       semantically unprocessable).
 *   <li>An {@link IllegalStateTransitionException} is a conflict with the current state, so it
 *       defaults to <strong>409</strong>.
 *   <li>Bean Validation failures map to 400 with field-level errors — both the request-body path
 *       ({@link MethodArgumentNotValidException}) and the method-parameter path ({@code @Validated}
 *       on {@code @RequestParam}/ {@code @PathVariable}, raised as {@link
 *       HandlerMethodValidationException} since Spring 6.1); not-found to 404; anything else to a
 *       500 that does not leak the exception message.
 *   <li>Spring MVC's protocol-level faults map to their HTTP meaning rather than the 500 fallback:
 *       an unmatched route ({@link NoResourceFoundException}) to <strong>404</strong>, a wrong
 *       method ({@link HttpRequestMethodNotSupportedException}) to <strong>405</strong>, an
 *       unsupported/unacceptable media type to <strong>415</strong>/<strong>406</strong>, and a
 *       malformed request body ({@link HttpMessageNotReadableException}) to <strong>400</strong>.
 *       These are more specific than the catch-all, so they win handler selection.
 * </ul>
 *
 * <p>The application-layer exceptions are handled by {@link ApplicationExceptionAdvice}, and
 * bean-validation {@code ConstraintViolationException} by {@link ConstraintViolationAdvice}, each
 * wired only when its types are on the classpath.
 */
...
public class AipersimmonDddWebExceptionHandler {
  ...
  /**
   * No handler matches the requested path → 404 (neutral detail; does not name the routing
   * mechanism).
   */
  ...
  /**
   * Body present but unparseable (e.g. malformed JSON) → 400; detail is neutral to avoid leaking
   * parser internals.
   */

...
/**
 * Applies a rate-limit policy per bucket key (client IP or a header). When quota remains the
 * request proceeds with {@code RateLimit} headers advertising the remaining allowance; when
 * exhausted it is rejected with {@code 429}, a {@code Retry-After} header, and a problem body —
 * following RFC 6585 / RFC 9110 and the IETF RateLimit header draft.
 */
public class RateLimitFilter extends OncePerRequestFilter {
  ...
  public RateLimitFilter(
    ...
    this.config = config;
    this.policy =
        new RateLimitPolicy(config.getPolicyName(), config.getLimit(), config.getWindow());
  }
  ...
  private Map<String, String> rateLimitHeaders(RateLimiter.Decision decision) {
    ...
    long resetSeconds =
        Math.max(0, decision.resetAt().getEpochSecond() - System.currentTimeMillis() / 1000);
    if ("ietf".equalsIgnoreCase(mode) || "both".equalsIgnoreCase(mode)) {
      headers.put(
          "RateLimit",
          "\"" + policy.name() + "\";r=" + decision.remaining() + ";t=" + resetSeconds);
      headers.put(
          "RateLimit-Policy",
          "\"" + policy.name() + "\";q=" + policy.limit() + ";w=" + policy.window().toSeconds());
    }

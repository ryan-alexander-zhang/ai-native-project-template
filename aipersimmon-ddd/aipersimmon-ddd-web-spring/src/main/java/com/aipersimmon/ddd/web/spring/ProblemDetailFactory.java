...
/**
 * Builds RFC 9457 {@link ProblemDetail}s for the advices, centralising the one piece of logic they
 * share: turning the {@link ErrorCode} an exception carries from the domain into a full wire
 * representation.
 *
 * <ul>
 *   <li><strong>Coded</strong> exceptions (an {@link ApiException}, or a {@code -core}/{@code
 *       -application} exception carrying an {@link ErrorCode}) resolve through the {@link
 *       ProblemRegistry} to a {@link ProblemDescriptor} — a per-code override if registered,
 *       otherwise the code's category family. A coded error therefore always carries a meaningful
 *       {@code type} (never {@code about:blank}) and its stable {@code code} extension.
 *   <li><strong>Code-less</strong> failures (a bare {@code DomainException}, bean validation,
 *       not-found, or an unexpected fault) fall back to {@code about:blank} with the
 *       caller-supplied status — {@code about:blank} is correct there, as the problem has no
 *       semantics beyond its HTTP status.
 * </ul>
 */
  ...
  private ProblemDetail coded(ErrorCode code, String detail, List<FieldError> errors) {
    ...
    ApiError error =
        ApiError.from(
            descriptor,
            code.code(),
            titleResolver.resolve(descriptor.titleKey()),
            detail,
            null,
            currentRequestId(),
            currentTraceId(),

...
/**
 * In-memory {@link ReplayGuard} for single-node and development use. A nonce is
 * remembered until its expiry; reuse within that window is reported as seen. Not
 * suitable for multiple instances — a store backend module replaces this bean.
 */
public class InMemoryReplayGuard implements ReplayGuard {
    ...
    public boolean seenBefore(String nonce, Duration ttl) {
        ...
        Instant previous = seen.compute(nonce, (k, current) ->
                (current != null && current.isAfter(now)) ? current : expiresAt);

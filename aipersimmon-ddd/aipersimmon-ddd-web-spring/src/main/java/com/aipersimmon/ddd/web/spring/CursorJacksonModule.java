...
/**
 * Serializes a {@link Cursor} as its opaque string value (not a nested object), so
 * a {@code Slice}/{@code Page} renders as {@code {"items":[...],"nextCursor":"..."}}
 * with the cursor a plain token. Registered as a Jackson {@code Module} bean, which
 * Boot's object-mapper builder picks up automatically.
 */
public class CursorJacksonModule extends SimpleModule {
    ...
    public CursorJacksonModule() {
        ...
        addSerializer(Cursor.class, new JsonSerializer<>() {
            @Override
        ...
        addDeserializer(Cursor.class, new JsonDeserializer<>() {
            @Override

...
/**
 * Maps a Bean Validation {@link ConstraintViolationException} — raised by the command bus's
 * validation interceptor, or by method-level {@code @Validated} — to a 400 with field-level {@link
 * FieldError}s, using the same wire shape as the MVC {@code MethodArgumentNotValidException} path.
 * Without this the exception would fall through to the 500 handler. Registered only when the Bean
 * Validation API is on the classpath.
 */
...
public class ConstraintViolationAdvice {
  ...
  public ProblemDetail handleConstraintViolation(ConstraintViolationException ex) {
    List<FieldError> errors =
        ex.getConstraintViolations().stream().map(ConstraintViolationAdvice::toFieldError).toList();

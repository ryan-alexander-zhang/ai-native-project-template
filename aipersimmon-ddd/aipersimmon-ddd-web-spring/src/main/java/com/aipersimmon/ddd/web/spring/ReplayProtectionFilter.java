...
/**
 * Rejects a captured, already-signed request that is replayed: it verifies the
 * signature, checks the timestamp is within tolerance, and — when nonce dedup is
 * enabled — rejects a nonce seen before. This is a security control, distinct from
 * idempotency. Failures are written as {@code application/problem+json} directly,
 * since the filter runs before the dispatcher.
 */
public class ReplayProtectionFilter extends OncePerRequestFilter {
    ...
    public ReplayProtectionFilter(RequestSignatureVerifier verifier, ReplayGuard replayGuard,
                                  ProblemHttpResponseWriter problemWriter, Clock clock, Duration tolerance,
                                  String signatureHeader, String timestampHeader,
                                  boolean nonceEnabled, String nonceHeader) {
    ...
    private void reject(HttpServletResponse response, String detail) throws IOException {
        problemWriter.write(response, HttpStatus.UNAUTHORIZED, "/problems/replay-rejected", detail, Map.of());
    }

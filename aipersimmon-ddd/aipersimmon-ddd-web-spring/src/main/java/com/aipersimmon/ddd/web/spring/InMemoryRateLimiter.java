...
/**
 * In-memory fixed-window {@link RateLimiter} for single-node and development use. Counts requests
 * per key within aligned windows. Fixed windows are simple but allow bursts at window boundaries;
 * for production (and multi-instance) use a store backend module, which replaces this bean with a

...
/**
 * Drives the web starter end to end through MockMvc: the advice renders each exception family to an
 * RFC 9457 problem body with the corrected status semantics (business rule → 422, state conflict →
 * 409, not-found → 404, bean validation → 400). A coded error resolves through the two-tier
 * registry — a per-code override to its own problem type, or its category family — and so never
 * renders {@code about:blank}; an uncoded failure legitimately does. A Slice serialises with its

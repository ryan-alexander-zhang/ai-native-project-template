...
/**
 * Redis-backed fixed-window {@link RateLimiter}: a per-(key, window) counter is incremented with
 * {@code INCR} and given the window as its TTL on first hit, so counting is atomic and shared
 * across instances. Fixed windows still allow boundary bursts, but the increment itself is
 * race-free.
 */

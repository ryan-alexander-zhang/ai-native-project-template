...
/**
 * Durable {@link DeadlineScheduler} that stores pending deadlines in a database
 * table and fires them from a scheduled poll — the same reliable "persist a row,
 * poll it, act, delete it" mechanism the transactional outbox uses, applied to
 * timeouts instead of messages. Unlike the in-process scheduler, pending deadlines
 * survive a restart (they are rows, not heap timers) and can be picked up by any
 * instance polling the table.
 *
 * <p>{@link #schedule} inserts (or reschedules) a row keyed by correlation id and
 * name; {@link #cancel} deletes it. The {@link #poll()} method, run on a schedule,
 * loads every row whose fire time has passed, dispatches it to the
 * {@link DeadlineHandler}, and deletes it on success; a row whose handler throws is
 * left in place and retried on the next poll. Delivery is therefore at-least-once —
 * firing may be delayed by up to the poll interval, and (across instances) a
 * deadline may fire more than once — so the handler must be idempotent, which a
 * saga's guarded lifecycle already provides (a deadline for an already-ended saga
 * is a no-op).
 */
public class JdbcDeadlineScheduler implements DeadlineScheduler {
    ...
    /**
     * @param handler resolves the {@link DeadlineHandler} lazily (at fire time), so
     *                a process manager can arm deadlines through this scheduler while
     *                also being the handler, without a construction-time cycle
     */
    public JdbcDeadlineScheduler(JdbcTemplate jdbc, Supplier<DeadlineHandler> handler,
    ...
    public void poll() {
        List<Deadline> due = jdbc.query(
                SELECT_DUE,
                (rs, rowNum) -> new Deadline(
        ...
        for (Deadline deadline : due) {
            try {
                ...
                log.warn("deadline dispatch failed for correlationId={} name={}, will retry",
                        deadline.correlationId(), deadline.name(), e);
            }
        }
    }
}

...
/**
 * The provably-hopeless causes are permanent (so the relay dead-letters them without
 * burning retries); everything else is transient. Cause-chain wrapping does not hide a
 * permanent cause. This is the same permanent set the Kafka consumer treats as
 * not-retryable, so the two transports agree.
 */
class DefaultFailureClassifierTest {
    ...
    void unknownEventTypeIsPermanent() {
        assertEquals(Failure.PERMANENT,

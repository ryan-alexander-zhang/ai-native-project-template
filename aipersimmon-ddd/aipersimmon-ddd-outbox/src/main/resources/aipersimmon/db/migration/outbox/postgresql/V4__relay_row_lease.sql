-- Per-row lease for the relay's claim (PostgreSQL). The relay used to rely solely on a ShedLock
-- lock on its schedule, which meant a killed instance left that lock held for its whole lease and
-- no other instance polled at all -- delivery stopped everywhere. With the lease on the row
-- instead, every instance polls and they claim disjoint rows; a killed instance only holds back the
-- rows it had claimed, and only until lease_until passes.

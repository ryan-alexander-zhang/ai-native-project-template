-- Record where each event is going, decided in the writing transaction (MySQL 8+).
--
...
--
-- MySQL has no ADD COLUMN IF NOT EXISTS, so these are unguarded; Flyway's history table is what
-- makes them run exactly once.
--
...
ALTER TABLE aipersimmon_outbox ADD COLUMN destination VARCHAR(255);
ALTER TABLE aipersimmon_dead_letter ADD COLUMN destination VARCHAR(255);

...
/**
 * Computes how long to wait before the next delivery attempt: capped exponential
 * growth with jitter. The cap doubles per attempt ({@code base * 2^(attempts-1)}) up to
 * a ceiling, so a persistently failing message backs off instead of hammering the broker
 * every poll (the flaw this replaces: a fixed retry with no spacing). Jitter (equal
 * jitter — half the cap plus a random half) spreads the retries of many messages that
 * failed together, so they do not all re-attempt in lockstep and stampede a recovering
 * broker.
 *
...
public final class RetryBackoff {
    ...
    /**
     * The delay before attempt number {@code attempts + 1}, given {@code attempts} have
     * already failed. Returns a value in {@code [cap/2, cap]} where
     * {@code cap = min(maxMillis, baseMillis * 2^(attempts-1))}.
     *

 ...
 * the default only logs; setting {@code aipersimmon.ddd.outbox.dispatch=in-process} switches to
 * republishing events in process instead. A messaging starter (for example {@code
 * -messaging-kafka}) can order itself before this class to register a broker-backed dispatcher that
 * wins over both defaults here — including when the in-process property is also set, which is why
 * the in-process bean is itself guarded by {@code @ConditionalOnMissingBean} — and it works with
 * any storage backend because this dispatch wiring carries no persistence. To deliver an event more
 * than one way (fan-out) or route by type, define your own {@code OutboxDispatcher} bean that
 * composes the others; all beans here back off.
 *

 ...
 *
 * <p>{@link #captureCurrent()} injects the active context into a carrier via the W3C propagator and
 * returns the resulting {@code traceparent}/{@code tracestate}. {@link #restore(String, String,
 * String)} extracts the stored context and starts a span that <em>links</em> back to it — a link,
 * not a parent, because a durable relay dispatches long after (and fanned out from) the creating
 * operation, so a parent-child edge would distort latency and hierarchy.
 */
public final class OpenTelemetryStoreAndForwardTracer implements StoreAndForwardTracer {
  ...
  public OpenTelemetryStoreAndForwardTracer(
      io.opentelemetry.api.trace.Tracer tracer, TextMapPropagator propagator) {
    this.tracer = tracer;

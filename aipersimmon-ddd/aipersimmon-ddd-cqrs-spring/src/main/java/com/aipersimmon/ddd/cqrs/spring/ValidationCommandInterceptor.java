...
/**
 * Validates each command against its Bean Validation constraints before the handler runs, throwing
 * {@link ConstraintViolationException} if any fail — so a malformed command is rejected before it
 * starts a transaction. Ordered between logging and transaction. Wired only when a Bean Validation

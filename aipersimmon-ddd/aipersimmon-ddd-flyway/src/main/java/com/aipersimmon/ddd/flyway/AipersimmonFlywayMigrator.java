 ...
 *
 * <p>Migrations ship with their owning module at
 * {@code classpath:aipersimmon/db/migration/{component}/{vendor}/V*.sql} — deliberately NOT under
 * {@code db/migration}, so Spring Boot's default Flyway (which scans {@code classpath:db/migration})
 * never sees them and never trips over the multiple {@code V1}s. This runner resolves the database
 * vendor from the {@link DataSource}, scans the classpath for the component sets present for that
 * vendor, and migrates each into its own history table ({@code <prefix><component>}). Being
 ...
 *
 * <p>It is invoked from a {@link org.springframework.boot.autoconfigure.flyway.FlywayMigrationStrategy}
 * AFTER the consumer's own default Flyway migrations have run, so aipersimmon tables never collide
 * with — nor share a version history with — the consumer's own schema.
 */
public final class AipersimmonFlywayMigrator {
    ...
    public void migrate(DataSource dataSource) {
        ...
        for (String component : components) {
            ...
            log.info("aipersimmon-ddd Flyway: component '{}' applied {} migration(s) from {} via history table '{}'",
                    component, result.migrationsExecuted, location, historyTable);
        }
    }
    ...
    private TreeSet<String> discoverComponents(String vendor) {
        ...
        Pattern pattern = Pattern.compile(Pattern.quote(BASE) + "/([^/]+)/" + Pattern.quote(vendor) + "/");
        PathMatchingResourcePatternResolver resolver =
                new PathMatchingResourcePatternResolver(getClass().getClassLoader());
        try {
            Resource[] resources = resolver.getResources("classpath*:" + BASE + "/*/" + vendor + "/*.sql");
            ...
            throw new IllegalStateException("Failed to scan the classpath for aipersimmon-ddd migrations", ex);
        }
        return components;
    }
    ...
    private String resolveVendor(DataSource dataSource) {
        ...
        try (Connection connection = dataSource.getConnection()) {
            ...
            throw new IllegalStateException("Cannot determine the database vendor for aipersimmon-ddd Flyway migration", ex);
        }
        ...
        throw new IllegalStateException("Unsupported database for aipersimmon-ddd Flyway migration: '" + product

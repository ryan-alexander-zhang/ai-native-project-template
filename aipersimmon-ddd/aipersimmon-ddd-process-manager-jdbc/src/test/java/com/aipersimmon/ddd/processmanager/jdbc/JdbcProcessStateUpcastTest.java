...
class JdbcProcessStateUpcastTest {
    ...
    void setUp() {
        ...
        runtime = new JdbcProcessRuntime(
                ...
                // The old v1 definition is kept (inactive) alongside the new active v2, for pinned instances.
                ...
                // Both schema codecs registered: v1 still reads live schema-1 instances, v2 serves new ones.
    ...
    void aPinnedOldSchemaInstanceIsUpcastOnReadAndStaysOnItsSchema() {
        ...
        jdbc.update("""
                ...
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""",
                "inst-1", TYPE.value(), ORDER.value(), "v1", 1,
        ...
        assertEquals("upcast:OLD-advanced", decodedState(),
    ...
    private String decodedState() {
        String stored = jdbc.queryForObject(
                "SELECT state_payload FROM aipersimmon_process_instance", String.class);
    ...
    private static final class DefinitionV1 implements ProcessDefinition<V> {
        ...
        public ProcessDecision<V> react(V state, ProcessInput input, ProcessContext context) {
            return new ProcessDecision<>(new V(state.data() + "-advanced"), ProcessLifecycle.RUNNING,
                    new ProcessStep("S2"), Optional.empty(), new DecisionCode("advanced"), List.of());
        }
    }
    ...
    private static final class DefinitionV2 implements ProcessDefinition<V> {
        ...
        @Override
        public ProcessDecision<V> start(ProcessInput input, ProcessContext context) {
            return new ProcessDecision<>(new V("fresh"), ProcessLifecycle.RUNNING, new ProcessStep("S1"),
                    Optional.empty(), new DecisionCode("started"), List.of());
        }
    ...
    private static ProcessStateCodec<V> stateCodecV1() {
        return new ProcessStateCodec<>() {
            ...
            public EncodedPayload encode(V state) {
                return new EncodedPayload(new PayloadType("upcast.state", 1),
                        state.data().getBytes(StandardCharsets.UTF_8));
            }
    ...
    private static ProcessStateCodec<V> stateCodecV2() {
        return new ProcessStateCodec<>() {
            ...
            @Override
            public EncodedPayload encode(V state) {
                return new EncodedPayload(new PayloadType("upcast.state", 2),

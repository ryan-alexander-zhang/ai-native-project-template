...
/**
 * The data for one appended row of the append-only transition log. {@code fromLifecycle}
 * and {@code fromStep} are empty on the first (start) transition. The encoded input is
 * carried as its logical payload type/version plus base64-persisted bytes.
 */

...
public final class EffectDispatcherRegistry {
    ...
    private final Map<ProcessEffectKind, ProcessEffectDispatcher> byKind = new EnumMap<>(ProcessEffectKind.class);
        ...
        for (ProcessEffectDispatcher dispatcher : dispatchers) {
            ...
            if (existing != null) {
                throw new IllegalStateException(
                        "two dispatchers registered for effect kind " + dispatcher.kind()
                                + ": " + existing.getClass().getName() + " and " + dispatcher.getClass().getName());
            }
        }
    }

...
/**
 * Reads and writes {@code aipersimmon_process_deadline}. This slice persists scheduling and
 * cancellation in the advance transaction; claiming and firing due deadlines is a later slice (the
...
public final class JdbcProcessDeadlineStore {
  ...
  /**
   * Cancel the current (highest) generation of a name while it is still live — {@code PENDING} or
   * already claimed {@code IN_FLIGHT}. Covering {@code IN_FLIGHT} is what lets a {@code
   * CancelDeadline} win against a timer a worker has already picked up: the deadline worker
   * re-checks the status under its lock before firing, so a cancel that lands first turns the fire
   * into an auditable no-op.
   */
  ...
  public Optional<DeadlineStatus> statusForUpdate(String deadlineId) {
    return jdbc
        .query(
            ...
            (rs, n) -> DeadlineStatus.valueOf(rs.getString("status")),
            deadlineId)
        .stream()
        .findFirst();
  }

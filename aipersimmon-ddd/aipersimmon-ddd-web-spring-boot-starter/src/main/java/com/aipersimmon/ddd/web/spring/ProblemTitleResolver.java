...
/**
 * Resolves a {@link com.aipersimmon.ddd.web.error.ProblemDescriptor}'s {@code titleKey} to a
 * human-readable title through the application {@link MessageSource}, using the request locale.
 * Falls back to the key itself when no message source is present or the key is not defined, so a

...
/**
 * The stable span-attribute catalog. Keeping the keys in one framework-free place lets
 * every span carry the same business dimensions so traces are queryable by them, and
 * keeps them aligned with OpenTelemetry semantic conventions where an equivalent
 * convention exists (the {@code messaging.*} keys mirror the OTEL messaging spec).
 *
 * <p>Business payloads never become attributes — only identity and routing metadata,
 * consistent with "metadata lives beside the payload, not inside it". Sensitive fields
 * must be masked by the caller before being set.
 */

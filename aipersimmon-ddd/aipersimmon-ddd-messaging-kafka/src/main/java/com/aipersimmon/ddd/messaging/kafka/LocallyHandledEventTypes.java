 ...
 * {@code @EventListener(EventEnvelope<T>)} for. The consumer bridge uses it to short-circuit:
 * a consumed record whose type nobody handles is dropped <strong>before</strong> the inbox
 * write / reconstruct / republish, because with no handler there is no side effect to make
 * atomic with the inbox — so there is nothing to record and nothing to redeliver.
 *
 * <p><strong>Safety is asymmetric and the scan is deliberately conservative.</strong> Skipping
 * an event that <em>is</em> handled would silently lose it, so the rule is "skip only when we
 * are certain nothing handles it": {@link #handlesAll()} is {@code true} (⇒ never skip) whenever
 * a listener could match more than one concrete type — a raw or wildcard {@code EventEnvelope},
 * a supertype like {@code Object}, a {@code classes()}-declared listener we cannot read the
 * generic of, or a payload type we cannot resolve to a well-formed {@link
 ...
 * for applications that consume via a mechanism this scan cannot see (a programmatic
 * {@code ApplicationListener}) is the {@code consumer.skip-locally-unhandled=false} property,
 * which supplies {@link #handlingEverything()}.
 */
final class LocallyHandledEventTypes {
    ...
    /**
     * Scans every bean's methods for {@code @EventListener} (including meta-annotated ones such
     * as {@code @TransactionalEventListener}) and narrows the handled set from the ones that
     * unambiguously take a concrete {@code EventEnvelope<ConcreteEvent>}; anything ambiguous
     * makes the result {@link #handlesAll()}. Reads bean <em>types</em> (not instances), so it
     * does not force eager initialization.
     */
    static LocallyHandledEventTypes scan(ConfigurableListableBeanFactory beanFactory) {
        ...
        for (String beanName : beanFactory.getBeanDefinitionNames()) {
            ...
            var methods = MethodIntrospector.selectMethods(ClassUtils.getUserClass(beanType),
                    (MethodIntrospector.MetadataLookup<EventListener>) m ->
                            AnnotatedElementUtils.findMergedAnnotation(m, EventListener.class));
    ...
    /**
     * Adds the concrete type this listener handles to {@code handled}, or returns {@code false}
     * if the listener could match more than one type (so the caller must fall back to
     * {@link #handlingEverything()}). A listener whose parameter cannot receive an
     * {@code EventEnvelope} at all is unrelated and returns {@code true} without adding anything.
     */
    private static boolean narrow(Method method, EventListener annotation, Set<Key> handled) {
        ...
        if (paramClass == EventEnvelope.class) {
            ...
            if (payload != null && payload != IntegrationEvent.class

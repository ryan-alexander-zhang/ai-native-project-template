 ...
 * handlers. A record must carry well-formed required CloudEvents attributes: {@code ce_id}
 * (the inbox key), {@code ce_type} (the catalog key), {@code ce_source}, {@code ce_specversion}
 * (must be {@code 1.0}), and {@code ce_dataschemaversion} (an integer {@code >= 1}, the other
 * half of the catalog key); {@code ce_time} is optional (it falls back to the broker record
 * timestamp, then the local clock). A record missing a required attribute, or carrying an
 * unparseable one, is rejected as a {@link MalformedIntegrationEventException} (a permanent
 * failure, dead-lettered) rather than defaulted around or retried — fabricating identity or
 * a schema version would defeat the inbox and the {@code (type, version)} contract.
...
public class KafkaIntegrationEventListener {
    ...
    private void requireSpecVersion(ConsumerRecord<String, String> record) {
        ...
        if (!IntegrationEventHeaders.SPEC_VERSION_VALUE.equals(specVersion)) {
            throw new MalformedIntegrationEventException(
                    "unsupported ce_specversion '" + specVersion + "'; this consumer speaks CloudEvents "
                            + IntegrationEventHeaders.SPEC_VERSION_VALUE);
        }
    }

    /** ce_dataschemaversion is required, an integer, and {@code >= 1} — it is half of the
     *  {@code (type, version)} catalog key, so a missing or bad value cannot be defaulted. */
    ...
    /** ce_time is optional; when absent, fall back to the broker record timestamp (a
     *  receive-time approximation), then to the local clock. A present but unparseable
     *  value is malformed. */

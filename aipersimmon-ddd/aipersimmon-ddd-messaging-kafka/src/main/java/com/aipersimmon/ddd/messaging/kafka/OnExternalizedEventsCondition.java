 ...
 * Gates the consumer bridge: with zero externalized events there is no topic to subscribe to, so
 * registering a {@code @KafkaListener} (which requires a non-empty topic set) would fail startup —
 * instead the bridge is simply not registered and the transport stays idle (see the idle WARN in

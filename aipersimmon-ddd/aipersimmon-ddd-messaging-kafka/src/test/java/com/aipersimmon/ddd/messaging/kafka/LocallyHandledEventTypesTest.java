 ...
 * {@code @EventListener(EventEnvelope<ConcreteEvent>)} methods, and falls back to
 * handle-everything (never skip) whenever a listener could match more than one type. Uses a bare
 * {@link DefaultListableBeanFactory} so only the registered beans are scanned (no framework beans
 * with their own {@code @EventListener}s to perturb the result).
 */
class LocallyHandledEventTypesTest {
    ...
    void narrowsToTheConcreteTypesWithAnEventListener() {
        LocallyHandledEventTypes handled = LocallyHandledEventTypes.scan(beanFactoryWith(AlphaHandler.class));
    ...
    void listenersForUnrelatedEventsAndClassesDeclarationsDoNotWiden() {
        LocallyHandledEventTypes handled = LocallyHandledEventTypes.scan(
    ...
    void nothingHandledWhenNoListeners() {
        LocallyHandledEventTypes handled = LocallyHandledEventTypes.scan(beanFactoryWith(PlainBean.class));

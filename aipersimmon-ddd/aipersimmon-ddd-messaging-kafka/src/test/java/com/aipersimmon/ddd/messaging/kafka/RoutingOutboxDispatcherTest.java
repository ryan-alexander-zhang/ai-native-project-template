...
/**
 * Verifies the router sends {@code @Externalized} events to their topic (and only there —
 * no in-process double-delivery) and keeps LOCAL events in process, keyed by the message's
 * {@code (type, version)}.
 */
class RoutingOutboxDispatcherTest {
    ...
    private static OutboxMessage message(String type, int version) {
        return new OutboxMessage("evt-1", "/ordering", type, version,
                "{}", Instant.parse("2026-01-01T00:00:00Z"), "o-1", "corr-1", "cause-1");
    }
}

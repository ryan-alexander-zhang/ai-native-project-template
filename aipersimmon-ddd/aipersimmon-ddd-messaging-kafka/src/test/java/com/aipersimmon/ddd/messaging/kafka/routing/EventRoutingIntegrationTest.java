...
/**
 * End-to-end over an in-JVM embedded broker, proving per-event routing (design-00006):
 * an {@code @Externalized} event goes to <em>its</em> topic and comes back through the
 * consumer bridge to local handlers <strong>exactly once</strong> (no in-process
 * double-delivery); events externalized to different targets land on different topics; and
 * a LOCAL event (no {@code @Externalized}) is delivered in process and never touches the
 * broker. This test lives in its own package so its auto-configuration scan sees only its
 * own fixtures.
 */
@SpringBootTest(
...
class EventRoutingIntegrationTest {
    ...
    private void awaitSize(List<String> list, int size, String label) throws InterruptedException {
        ...
        throw new AssertionError("timed out waiting for " + size + " delivery of " + label
                + " (saw " + list.size() + ")");
    }
    ...
    private static OutboxMessage message(String id, String type, String payload) {
        return new OutboxMessage(id, "/routing-test", type, 1, payload,
                Instant.parse("2026-01-01T00:00:00Z"), "agg-" + id, id, null);
    }
}

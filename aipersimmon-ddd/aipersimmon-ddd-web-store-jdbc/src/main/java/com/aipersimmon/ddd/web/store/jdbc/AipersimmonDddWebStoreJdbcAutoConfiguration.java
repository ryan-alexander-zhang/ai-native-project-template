...
/**
 * Wires the JdbcTemplate-backed web stores once a {@code JdbcTemplate} is present.
 * Each bean is {@code @ConditionalOnMissingBean} on its SPI type, so it replaces
 * the {@code -web-spring} in-memory default while still yielding to a consumer's
 * own implementation.
 */
...
public class AipersimmonDddWebStoreJdbcAutoConfiguration {
    ...
    public IdempotencyStore jdbcIdempotencyStore(JdbcTemplate jdbc, ObjectProvider<ObjectMapper> objectMapper,
                                                 Clock aipersimmonDddWebStoreClock) {
        return new JdbcIdempotencyStore(jdbc, objectMapper.getIfAvailable(ObjectMapper::new),

...
/**
 * JdbcTemplate-backed {@link IdempotencyStore}: the first response for a key is
 * inserted into {@code aipersimmon_web_idempotency} and replayed on retry.
 * {@link #saveIfAbsent} relies on the primary key for atomicity — a duplicate
 * insert loses — after purging any expired entry for the key.
 */
public class JdbcIdempotencyStore implements IdempotencyStore {
    ...
    public Optional<StoredResponse> find(String key) {
        List<StoredResponse> found = jdbc.query(
                ...
                key, Timestamp.from(clock.instant()));
    ...
    public boolean saveIfAbsent(String key, StoredResponse response, Duration ttl) {
        ...
        jdbc.update("DELETE FROM aipersimmon_web_idempotency WHERE idempotency_key = ? AND expires_at <= ?",
                key, Timestamp.from(now));
        try {
            jdbc.update("INSERT INTO aipersimmon_web_idempotency "
                            + "(idempotency_key, response_status, response_body, response_headers, created_at, expires_at) "
                            + "VALUES (?, ?, ?, ?, ?, ?)",
                    key, response.status(), response.body(), writeHeaders(response.headers()),

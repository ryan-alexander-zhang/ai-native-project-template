...
/**
 * JdbcTemplate-backed fixed-window {@link RateLimiter}: per (key, window) counters
 * live in {@code aipersimmon_web_rate_limit}, shared across instances. Adequate but
 * not optimal — fixed windows permit boundary bursts and the increment is not a
 * single atomic statement across all databases; prefer the Redis backend under
 * high concurrency.
 */
public class JdbcRateLimiter implements RateLimiter {
    ...
    public Decision tryAcquire(String key, RateLimitPolicy policy) {
        ...
        jdbc.update("DELETE FROM aipersimmon_web_rate_limit WHERE bucket_key = ? AND window_start < ?",
                key, windowStart);
        ...
        if (updated == 0) {
            try {
                jdbc.update("INSERT INTO aipersimmon_web_rate_limit (bucket_key, window_start, count) VALUES (?, ?, 1)",
                        key, windowStart);
            } catch (DuplicateKeyException e) {
                jdbc.update("UPDATE aipersimmon_web_rate_limit SET count = count + 1 "
                        + "WHERE bucket_key = ? AND window_start = ?", key, windowStart);
            }
        }

        Long count = jdbc.queryForObject(
                "SELECT count FROM aipersimmon_web_rate_limit WHERE bucket_key = ? AND window_start = ?",
                Long.class, key, windowStart);

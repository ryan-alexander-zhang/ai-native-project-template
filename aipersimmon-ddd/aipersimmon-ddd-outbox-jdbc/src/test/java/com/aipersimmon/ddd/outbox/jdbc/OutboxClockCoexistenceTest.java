 ...
 * already contributes a {@code Clock} bean, this starter must still contribute its own
 * {@code outboxClock} and wire {@code outboxWriter} to it — not back off and leave the by-name
 * injection dangling.
 *
 * <p>Guards both root causes at once: the name-scoped {@code @ConditionalOnMissingBean(name =
 * "outboxClock")} keeps {@code outboxClock} present despite the foreign clock, and {@code -parameters}
 * (compiler flag on the library parent) lets {@code outboxWriter}'s {@code Clock outboxClock} parameter
 * resolve by name across the two candidates. Before the fix, {@code outboxClock} backed off (type-scoped
 * condition) so this context could not wire {@code outboxWriter} deterministically.
 */
class OutboxClockCoexistenceTest {
    ...
    void keepsItsOwnNamedClockAlongsideAnotherComponentsClock() {
        runner.run(context -> {
            ...
            // outboxWriter (the IntegrationEvents port) wired successfully — its Clock resolved by name.

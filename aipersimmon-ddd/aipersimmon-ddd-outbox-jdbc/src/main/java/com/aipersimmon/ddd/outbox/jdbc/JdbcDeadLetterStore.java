...
/**
 * Backs the {@link DeadLetterStore} with an {@code aipersimmon_dead_letter} table that mirrors the
 * outbox row plus triage columns ({@code attempts}, {@code reason}, {@code last_error}, {@code
 * failed_at}). Every move runs in one {@link TransactionTemplate} transaction — the dead-letter
 * insert and the outbox delete commit together — so a message is never duplicated across the two
 * tables nor lost between them.
 */

...
public class OutboxRelay {
  ...
  public void relay() {
    ...
    List<Pending> batch =
        jdbc.query(SELECT_DUE, this::mapRow, maxAttempts, now, maxAttempts, now, batchSize);
   ...
   * every second). Instead it backs the row off, so the move is retried at the backoff cadence and
   * self-heals once the store recovers.
   *
  ...
  /**
   * A null or blank subject carries no per-aggregate ordering key (matching the SELECT and the
   * Kafka partition key), so it never blocks or is blocked.
   */

        ...
        for (ProcessPayloadCodec<?> codec : codecs) {
            ...
            if (byType != null) {
                throw new IllegalStateException(
                        "two payload codecs for " + type.logicalType() + "/v" + type.version()
                                + ": " + byType.getClass().getName() + " and " + codec.getClass().getName());
            }
            ProcessPayloadCodec<?> byJava = byJavaType.put(javaType, codec);
            if (byJava != null) {
                throw new IllegalStateException(
                        "two payload codecs for Java type " + javaType.getName()
                                + ": " + byJava.getClass().getName() + " and " + codec.getClass().getName());
            }
        }
    }

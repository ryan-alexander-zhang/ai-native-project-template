...
/**
 * The stable, versioned logical type of a persisted payload (a command, a deadline input, or an
 * integration-event body) — for example {@code ("ordering.fulfilment.reserve-stock", 1)}. It is the
 * persistence contract, so a Java class can be renamed without breaking stored rows; a class name
 * is never used as the type. For an integration-event body it must match the event's

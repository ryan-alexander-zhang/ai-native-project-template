...
/**
 * The read-only context the runtime hands a {@link ProcessDefinition} for one
 * decision. Supplying {@code now}, the process reference, and the causing
 * {@link CommandContext} from the runtime (rather than letting the definition read a
 * clock or mint ids) keeps decisions deterministic and repeatable in tests.
 *
 * <p>On {@code start} the current lifecycle and step are empty; on {@code react} both
 * are present. The definition treats this as read-only — it returns a new decision,
 * it does not mutate the context or the state passed alongside it.
 *
...
public record ProcessContext(
    ...
    public ProcessContext {
        ...
        if (currentLifecycle == null) {
            throw new IllegalArgumentException("currentLifecycle optional required (use Optional.empty())");
        }
        if (currentStep == null) {

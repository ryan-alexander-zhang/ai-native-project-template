        ...
        for (ProcessStateCodec<?> codec : codecs) {
            ...
            if (existing != null) {
                throw new IllegalStateException(
                        "two state codecs for " + key.processType().value()
                                + " schema v" + key.schemaVersion().value()
                                + ": " + existing.getClass().getName() + " and " + codec.getClass().getName());
            }
        }
    }
    ...
    public ProcessStateCodec<?> forState(ProcessType processType, StateSchemaVersion schemaVersion) {
        ...
        if (codec == null) {
            throw new ProcessSerializationException(
                    "no state codec registered for " + processType.value()

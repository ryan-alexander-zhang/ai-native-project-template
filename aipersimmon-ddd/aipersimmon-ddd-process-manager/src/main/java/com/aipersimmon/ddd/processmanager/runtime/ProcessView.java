...
/**
 * A read-only snapshot of a process instance for queries and operations: identity, pinned versions,
 * current lifecycle/step, terminal outcome when present, revision, and — when the instance is
 * suspended — the lifecycle it will resume to and why it was suspended. It exposes the runtime
 * metadata, not the decoded business state (which is the consumer's own concern).
 *
  ...
  public ProcessView {
    ...
    if (resumeLifecycle == null) {
      throw new IllegalArgumentException(
          "resumeLifecycle optional required (use Optional.empty())");
    }
    if (suspensionReason == null) {
      throw new IllegalArgumentException(
          "suspensionReason optional required (use Optional.empty())");
    }
    ...
    if (suspended != resumeLifecycle.isPresent() || suspended != suspensionReason.isPresent()) {
      throw new IllegalArgumentException(
          "resumeLifecycle and suspensionReason are present iff the lifecycle is SUSPENDED; got "
              + lifecycle
              + " with resumeLifecycle "
              + resumeLifecycle
              + " and suspensionReason present="
              + suspensionReason.isPresent());
    }
  }
}

...
/**
 * Thrown by {@code start} under the {@code reject} duplicate-business-key policy when a live
 * instance already exists for {@code (processType, businessKey)} but the start arrives with a
 * different input message id. Under the {@code fold} policy the start is folded into a duplicate
...
public final class ProcessAlreadyExistsException extends ProcessException {
  ...
  public ProcessAlreadyExistsException(ProcessType processType, ProcessBusinessKey businessKey) {
    super(
        "a process instance already exists for "
            + processType.value()
            + " / "
            + businessKey.value());

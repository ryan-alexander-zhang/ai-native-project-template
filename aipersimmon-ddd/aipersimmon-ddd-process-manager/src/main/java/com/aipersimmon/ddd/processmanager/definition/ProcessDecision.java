...
/**
 * The result a {@link ProcessDefinition} returns for one input: the new business
 * {@code state}, the target {@code lifecycle} and {@code step}, an {@code outcome}
 ...
 *   <li>a deadline name may appear in at most one deadline effect, so scheduling and
 *       cancelling the same name in one decision is not ambiguous.
 * </ul>
 * The remaining check — that the lifecycle transition itself is legal — needs the
 * current lifecycle and is performed by the runtime. Effects are defensively copied
 * and their order is preserved, so replaying the same input yields the same effects.
 *
...
public record ProcessDecision<S>(
    ...
    public ProcessDecision {
        ...
        if (lifecycle.isTerminal() && outcome.isEmpty()) {
            throw new IllegalArgumentException("a terminal lifecycle (" + lifecycle + ") must carry an outcome");
        }
        if (!lifecycle.isTerminal() && outcome.isPresent()) {
            throw new IllegalArgumentException("a non-terminal lifecycle (" + lifecycle + ") must not carry an outcome");
        }
    ...
    private static void requireUnambiguousDeadlines(List<ProcessEffect> effects) {
        ...
        for (ProcessEffect effect : effects) {
            DeadlineName name = switch (effect) {
            ...
            if (name != null && !seen.add(name)) {
                throw new IllegalArgumentException(
                        "ambiguous deadline operations for '" + name.value()

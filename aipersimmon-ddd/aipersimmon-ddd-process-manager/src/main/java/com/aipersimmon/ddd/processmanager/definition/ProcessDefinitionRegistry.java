...
/**
 * Indexes the registered {@link ProcessDefinition}s so the runtime can resolve the active version
 * for a new instance and the pinned version for a running one. It does not scan or reflect over
 * business classes; the definitions are supplied explicitly.
 *
 * <p>Construction fails fast when a {@code (processType, definitionVersion)} pair is registered
 * twice, or when a process type does not have exactly one version with {@link
 * ProcessDefinition#activeForNewInstances()} true.
 */
public final class ProcessDefinitionRegistry {

  private final Map<ProcessType, Map<DefinitionVersion, ProcessDefinition<?>>> byType =
      new HashMap<>();
  ...
  public ProcessDefinitionRegistry(Iterable<? extends ProcessDefinition<?>> definitions) {
    ...
    for (ProcessDefinition<?> definition : definitions) {
      ...
      if (existing != null) {
        throw new IllegalStateException(
            "two process definitions registered for "
                + type.value()
                + " "
                + version.value()
                + ": "
      ...
      if (definition.activeForNewInstances()) {
        ...
        if (priorActive != null) {
          throw new IllegalStateException(
              "more than one active definition for process type "
                  + type.value()
                  + ": "
    ...
    for (ProcessType type : byType.keySet()) {
      ...
      if (activeDefinition == null) {
        throw new IllegalStateException(
            "no active definition for process type "
                + type.value()

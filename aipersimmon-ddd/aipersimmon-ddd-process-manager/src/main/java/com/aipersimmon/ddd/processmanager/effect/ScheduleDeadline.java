...
/**
 * Effect asking the runtime to arm (or reschedule) the named timer {@code name} to
 * fire at {@code dueAt}. When it fires, the runtime turns {@code input} into an
 * ordinary {@link ProcessInput} and feeds it back through {@code handle} — a deadline
 * is not a separate callback. Rescheduling the same name bumps its generation so a
 * late fire of an old generation is a no-op.
 *

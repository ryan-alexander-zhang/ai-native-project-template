...
/**
 * Thrown when an optimistic-concurrency check fails: the instance's revision changed under a
 * concurrent transition. The runtime retries a bounded number of times (reload, re-decide) before
 * surfacing this to the message layer.
 */
public final class StaleProcessRevisionException extends ProcessException {
  ...
  public StaleProcessRevisionException(
      ProcessRef processRef, ProcessRevision expected, ProcessRevision actual) {
    super(
        "stale revision for instance "
            + processRef.instanceId().value()
            + ": expected "
            + expected.value()
            + " but found "
            + actual.value());

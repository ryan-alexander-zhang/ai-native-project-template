...
/**
 * A consumer's process, expressed as a pure, deterministic decision object: given the
 * current state, an input, and a read-only {@link ProcessContext}, it returns a
 * {@link ProcessDecision}. It must do no I/O — no repository, HTTP, command bus,
 * integration-event publish, system clock, randomness, Spring bean, or third-party
 * SDK — so it is fully unit-testable and safely replayable.
 *
 * <p>{@code start} handles only a new instance (context lifecycle/step empty);
 * {@code react} handles only an existing one (both present). It never mutates the
 * state passed in; it returns a new state inside the decision. Effect ids are not
 * created here — the runtime derives them from {@code transitionId + effectIndex}.
 *
 * <p>Several versions of one {@link #processType()} may be registered at once, but
 * exactly one has {@link #activeForNewInstances()} true; older versions stay
 * registered (with it false) to keep serving their running instances.
 *

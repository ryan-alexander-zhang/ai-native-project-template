...
/**
 * Effect asking the runtime to publish {@code event} through the outbox. Used to reach a bounded
 * context deployed as a separate service: the target consumes the event and its inbound adapter
 * translates it into a local command. The event's logical type/version comes from its
 * {@code @EventType}; a codec provides the payload serialization.
 *

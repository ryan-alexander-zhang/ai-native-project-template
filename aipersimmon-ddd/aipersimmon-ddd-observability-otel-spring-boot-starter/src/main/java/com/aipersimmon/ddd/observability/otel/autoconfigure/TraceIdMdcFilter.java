...
/**
 * Bridges the edge request id and the OpenTelemetry trace, in both directions, for the duration
 * of each request — so an operator can pivot between them:
 *
 * <ul>
 ...
 *       responses (alongside the edge {@code requestId}), and so log lines can carry it.</li>
 *   <li><strong>request id → span</strong>: stamps the edge request id (the {@code X-Request-Id} the
 *       caller was handed — client-supplied or server-generated, already resolved onto the MDC by
 *       web-spring's {@code RequestIdFilter}) onto the active server span as the {@code request.id}
 *       attribute, so the trace is <em>searchable by the id the caller holds</em>. This is the
 *       correlation-id pattern: a caller-facing id that is not the trace id must be indexed on the
 *       trace to be found; reading the resolved MDC value (rather than only capturing an inbound
 *       header) also covers the server-generated case, where no {@code X-Request-Id} header arrived.
 *       It is intentionally distinct from the messaging-layer {@code correlation.id} the command
 *       interceptor stamps.</li>
 * </ul>
 *

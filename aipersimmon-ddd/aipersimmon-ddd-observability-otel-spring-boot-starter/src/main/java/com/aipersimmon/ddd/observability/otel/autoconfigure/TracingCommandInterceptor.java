 ...
 * auto-instrumentation only sees HTTP/JDBC/Kafka, not the in-house command bus, so
 * without this a trace shows an HTTP span, a gap, then SQL spans with no indication of
 * which command ran.
 *
 * <p>Ordered outermost (further out than {@code LoggingCommandInterceptor} at 0) so the
 * span is active while the rest of the chain — logging, validation, transaction, handler —
 * runs, which lets those logs carry the span's trace id and puts the transaction inside
 * the command span. A thrown exception marks the span as failed before it propagates.
 */

...
/**
 * Verifies the auto-configuration wires the tracer and command-span interceptor when an {@code
 * OpenTelemetry} bean is present (supplied by the OTEL starter in a real app), and that a
 * consumer-provided tracer overrides the default.
 */
class AipersimmonDddObservabilityOtelAutoConfigurationTest {
  ...
  private final ApplicationContextRunner runner =
      new ApplicationContextRunner()
          .withConfiguration(
              AutoConfigurations.of(AipersimmonDddObservabilityOtelAutoConfiguration.class));
  ...
  void backsOffWhenConsumerProvidesTracer() {
    runner
        .withUserConfiguration(OpenTelemetryConfig.class, CustomTracerConfig.class)
        .run(
            context -> {
              assertThat(context).hasSingleBean(Tracer.class);
  ...
  static class CustomTracerConfig {
    static final Tracer CUSTOM =
        name ->
            new Tracer.SpanScope() {
              @Override

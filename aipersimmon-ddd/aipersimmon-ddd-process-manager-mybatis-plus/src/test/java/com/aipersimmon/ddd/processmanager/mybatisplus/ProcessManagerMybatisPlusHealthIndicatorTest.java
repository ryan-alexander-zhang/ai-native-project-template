...
class ProcessManagerMybatisPlusHealthIndicatorTest {
  ...
  void setUp() {
    ...
    ProcessBacklog backlog =
        new ProcessBacklog(
            stores.effects(), stores.deadlines(), stores.instances(), Clock.systemUTC());
    health =
        new ProcessManagerHealthIndicator(backlog, Duration.ofMinutes(15), Duration.ofSeconds(60));
  }

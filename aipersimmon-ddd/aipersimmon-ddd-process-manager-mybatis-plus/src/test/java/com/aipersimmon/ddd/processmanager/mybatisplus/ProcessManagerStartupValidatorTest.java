...
import com.aipersimmon.ddd.processmanager.engine.autoconfigure.ProcessManagerStartupValidator;
...
class ProcessManagerStartupValidatorTest {
  ...
  void setUp() {
    ...
    stores = ProcessStores.over(db);
    instances = stores.instances();
    definitions = new ProcessDefinitionRegistry(List.of(new MybatisTestProcess.Definition()));
    stateCodecs = new ProcessStateCodecRegistry(List.of(MybatisTestProcess.stateCodec()));
  }
  ...
  void failsWhenAnIntegrationEventCodecTypeDisagreesWithEventType() {
    ProcessPayloadCodecRegistry mismatched =
        new ProcessPayloadCodecRegistry(List.of(eventCodec(new PayloadType("wrong.name", 1))));
    IllegalStateException e =
        assertThrows(
            IllegalStateException.class,
  ...
  void passesWhenTheIntegrationEventCodecTypeMatchesEventType() {
    ProcessPayloadCodecRegistry matched =
        new ProcessPayloadCodecRegistry(
            List.of(eventCodec(new PayloadType("test.thing-happened", 1))));
  ...
  void failsWhenTheRelayIsEnabledButNoDispatcherIsRegistered() {
    IllegalStateException e =
        assertThrows(
            IllegalStateException.class,
            () ->
                validator(empty(), new EffectDispatcherRegistry(List.of()), true)
                    .afterPropertiesSet());

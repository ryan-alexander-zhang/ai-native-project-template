...
/**
 * A tiny process used by {@link MybatisProcessRuntimeTest}: enough inputs, effects, and a state to
 * exercise start, advance, terminal, compensation, an illegal transition, a failing decision
 * (rollback), and a scheduled deadline. Not a business sample.
 */
final class TestFulfilment {
  ...
  static final class Definition implements ProcessDefinition<State> {
    ...
    public ProcessDecision<State> react(State state, ProcessInput input, ProcessContext context) {
      return switch (input) {
        case Advance ignored ->
            new ProcessDecision<>(
                new State("S2", state.count() + 1),
                ProcessLifecycle.RUNNING,
                new ProcessStep("S2"),
                Optional.empty(),
        ...
        case EnterCompensating ignored ->
            new ProcessDecision<>(
                ...
                new ProcessStep("COMP"),
                Optional.empty(),
                new DecisionCode("compensating"),
                List.of());
        // From COMPENSATING, returning RUNNING is an illegal transition the runtime must reject.
        case IllegalBack ignored ->
            new ProcessDecision<>(
                new State("S1", state.count()),
                ProcessLifecycle.RUNNING,
                new ProcessStep("S1"),
                Optional.empty(),
                new DecisionCode("illegal"),
                List.of());
        case ArmDeadline ignored ->
            new ProcessDecision<>(
                new State("WAIT", state.count()),
                ProcessLifecycle.RUNNING,
                new ProcessStep("WAIT"),
                Optional.empty(),
                new DecisionCode("armed"),
                List.of(
                    new ScheduleDeadline(
                        new DeadlineName("REVIEW"), context.now(), new DeadlineFired())));
        case DeadlineFired ignored ->
            new ProcessDecision<>(
                new State("DONE", state.count()),
                ProcessLifecycle.COMPLETED,
                new ProcessStep("DONE"),
                Optional.of(new ProcessOutcome("REVIEW_EXPIRED")),
                new DecisionCode("deadline-fired"),
                List.of());
        case ArmPoisonDeadline ignored ->
            new ProcessDecision<>(
                new State("WAIT_POISON", state.count()),
                ProcessLifecycle.RUNNING,
                new ProcessStep("WAIT_POISON"),
                Optional.empty(),
                new DecisionCode("armed-poison"),
                List.of(
                    new ScheduleDeadline(new DeadlineName("POISON"), context.now(), new Boom())));
        case CancelReview ignored ->
            new ProcessDecision<>(
                new State("NO_WAIT", state.count()),
                ProcessLifecycle.RUNNING,
                new ProcessStep("NO_WAIT"),
                Optional.empty(),
        ...
        case FanOut ignored ->
            new ProcessDecision<>(
                ...
                new ProcessStep("FAN"),
                Optional.empty(),
                new DecisionCode("fanned"),
                List.of(
                    new DispatchCommand(new DoWork("first")),
                    new DispatchCommand(new DoWork("second"))));
        case MaxLifetimeExceeded ignored ->
            new ProcessDecision<>(
                new State("EXPIRED", state.count()),
                ProcessLifecycle.COMPLETED,
                new ProcessStep("EXPIRED"),
                Optional.of(new ProcessOutcome("MAX_LIFETIME")),
                new DecisionCode("max-lifetime"),
                List.of());
  ...
  static List<ProcessPayloadCodec<?>> payloadCodecs() {
    return List.of(
        ...
        payloadCodec(
            "test.enter-compensating",
            EnterCompensating.class,
            e -> "",
            s -> new EnterCompensating()),
  ...
  static ProcessStateCodec<State> stateCodec() {
    return new ProcessStateCodec<>() {
      ...
      public EncodedPayload encode(State state) {
        ...
        return new EncodedPayload(
            new PayloadType("test.proc.state", SCHEMA.value()),

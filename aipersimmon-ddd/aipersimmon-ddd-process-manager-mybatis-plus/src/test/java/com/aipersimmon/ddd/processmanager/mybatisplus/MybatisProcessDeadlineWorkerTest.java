...
class MybatisProcessDeadlineWorkerTest {
  ...
  private MybatisProcessDeadlineStore deadlineStore;
  ...
  void setUp() {
    ...
    MybatisProcessEffectStore effectStore = stores.effects();
    deadlineStore = stores.deadlines();
  ...
  void aSupersededGenerationIsAnAuditableNoOp() {
    ...
    runtime.handle(
        started.processRef(),
        new TestFulfilment.ArmDeadline(),
        CommandContext.root(Tenants.ROOT, "arm-1"));
    runtime.handle(
        started.processRef(),
        new TestFulfilment.ArmDeadline(),
        CommandContext.root(Tenants.ROOT, "arm-2"));
    assertEquals(
        2L,
        jdbc.queryForObject(
            "SELECT COUNT(*) FROM aipersimmon_process_deadline WHERE status = 'PENDING'",
            Long.class));
  ...
  void exhaustingFireRetriesMovesTheDeadlineToDeadAndSuspends() {
    ...
    assertEquals(
        ...
        jdbc.queryForObject("SELECT status FROM aipersimmon_process_deadline", String.class));
    var instance =
        jdbc.queryForMap("SELECT lifecycle, suspension_source FROM aipersimmon_process_instance");
  ...
  void aFiredDeadlineKeepsTheCorrelationOfTheFlowThatArmedIt() {
    ...
    var fired =
        jdbc.queryForMap(
            "SELECT correlation_id FROM aipersimmon_process_transition WHERE decision_code = 'deadline-fired'");
    assertEquals(
        "msg-arm",
        fired.get("CORRELATION_ID"),
        "the timeout fires under the arming flow's correlation");
  }
  ...
  void aDeadlineCancelledWhileInFlightDoesNotFire() {
    ...
    unitOfWork.execute(
        () ->
            stores
                ...
                .claimDueDeadlines(now, 10, "token-A", now.minusSeconds(1)));
    assertEquals(
        "IN_FLIGHT",
        jdbc.queryForObject("SELECT status FROM aipersimmon_process_deadline", String.class));
    ...
    runtime.handle(
        ...
        CommandContext.root(Tenants.ROOT, "msg-cancel"));

...
/** A minimal process used by the MyBatis-Plus Boot slice test. */
final class MybatisTestProcess {

  private MybatisTestProcess() {}

  static final ProcessType TYPE = new ProcessType("mybatis.test");
  ...
  static final class Definition implements ProcessDefinition<St> {
    ...
    public ProcessDecision<St> react(St state, ProcessInput input, ProcessContext context) {
      throw new UnsupportedOperationException("no react in the mybatis smoke test");
    }
  }

  static ProcessPayloadCodec<Begin> beginCodec() {
    return codec("mybatis.begin", Begin.class, Begin::reference, Begin::new);
  }

  static ProcessPayloadCodec<DoThing> doThingCodec() {
    return codec("mybatis.do-thing", DoThing.class, DoThing::reference, DoThing::new);
  }
    ...
    return new ProcessStateCodec<>() {
      ...
      public EncodedPayload encode(St state) {
        return new EncodedPayload(
            new PayloadType("mybatis.test.state", 1),

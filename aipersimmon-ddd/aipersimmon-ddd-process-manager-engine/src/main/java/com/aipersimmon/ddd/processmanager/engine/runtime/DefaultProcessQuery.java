...
/**
 * The read-only {@link ProcessQuery} over the four-table store. Beyond the single-instance {@link
 * ProcessView} it offers operational reads: paged search by
...
public final class DefaultProcessQuery implements ProcessQuery {
  ...
  public Optional<ProcessView> find(ProcessRef processRef) {
    return instances
        .find(processRef.instanceId())
        .map(
            row -> {
              // Fail fast on a ref that names a real instanceId but the wrong

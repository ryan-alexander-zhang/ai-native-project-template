 ...
 * skipped and the effect is fenced to a terminal {@code CANCELLED} state under its lease — no
 * charge/reserve command escapes. This makes operator cancel honour "no new external side effect
 * after cancel returns", not merely "cancel the not-yet-claimed effects".
 *
        ...
        try {
          dispatchers.dispatch(
              new DecodedProcessEffect(
                  effect.effectId(), effect.instanceId(), effect.kind(), payload),
              effect.context());

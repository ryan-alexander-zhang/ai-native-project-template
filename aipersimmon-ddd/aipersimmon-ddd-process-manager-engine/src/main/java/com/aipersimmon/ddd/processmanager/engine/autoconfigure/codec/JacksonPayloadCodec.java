...
/**
 * A Jackson-backed {@link ProcessPayloadCodec} the convenience layer generates from a {@link
 * ProcessSerializationCatalog} entry. The logical type/version and Java type are the catalog's
 * explicit registration, never the class name. Serialization failures are surfaced as {@link
 * ProcessSerializationException}.
 *
...
final class JacksonPayloadCodec<T> implements ProcessPayloadCodec<T> {
  ...
  public EncodedPayload encode(T value) {
    try {
      ...
      throw new ProcessSerializationException(
          "cannot encode payload of type " + type.logicalType(), e);
    }
  }
  ...
  public T decode(EncodedPayload payload) {
    try {
      ...
      throw new ProcessSerializationException(
          "cannot decode payload of type " + type.logicalType(), e);
    }
  }
}

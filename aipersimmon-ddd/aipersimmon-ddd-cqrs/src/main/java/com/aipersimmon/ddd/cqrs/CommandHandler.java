 ...
 * inside the handler.
 *
 * <p>The handler receives the command's {@link CommandContext} alongside the
 * command. Most handlers ignore it; a handler that emits an integration event
 * passes it on (see {@code IntegrationEvents.publish}) so the outbound event is
 * correlated to this command and records it as the cause.
 *

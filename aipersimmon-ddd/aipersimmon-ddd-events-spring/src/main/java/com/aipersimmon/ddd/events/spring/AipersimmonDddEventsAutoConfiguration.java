...
public class AipersimmonDddEventsAutoConfiguration {
    ...
    private static final Logger log = LoggerFactory.getLogger(AipersimmonDddEventsAutoConfiguration.class);
     ...
     *
     * <p>Note this publisher is <em>not</em> {@link com.aipersimmon.ddd.application.DurableIntegrationEvents}:
     * it does not persist events, so {@code @Externalized} events published through it never reach an
     * external broker — the Kafka messaging starter fails startup rather than degrade silently.
     */
    @Bean
    ...
    public IntegrationEvents integrationEvents(ApplicationEventPublisher publisher,
        ...
        log.info("aipersimmon-ddd integration-event transport: in-process (local) — no durable outbox on "

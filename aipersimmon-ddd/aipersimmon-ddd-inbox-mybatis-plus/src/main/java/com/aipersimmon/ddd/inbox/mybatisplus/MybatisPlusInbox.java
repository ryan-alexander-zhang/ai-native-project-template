...
/**
 * Records handled message keys through a MyBatis-Plus mapper, scoped to a configured
 ...
 * composite primary key still guards the rare race of two simultaneous first-time
 * deliveries: the losing insert fails and its transaction rolls back, so the message
 * is simply redelivered and then detected as already processed.
 *
...
public class MybatisPlusInbox implements Inbox {
    ...
    public boolean alreadyProcessed(String messageKey) {
        Long count = mapper.selectCount(new LambdaQueryWrapper<InboxRecord>()

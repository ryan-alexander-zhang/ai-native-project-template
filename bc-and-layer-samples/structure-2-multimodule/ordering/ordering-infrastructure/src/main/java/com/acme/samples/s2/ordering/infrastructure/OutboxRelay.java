...
/**
 * Polls the outbox and publishes unsent events to Kafka, then marks them sent.
 * At-least-once delivery; the consumer side is idempotent (inbox).
 */
@Component
public class OutboxRelay {
    ...
    public void flush() {
        List<OutboxPo> pending = outbox.selectList(
                new QueryWrapper<OutboxPo>().eq("sent", false).orderByAsc("id"));

-- Ordering context
create table if not exists s2_ordering.customers (
    id text primary key, name text not null, credit_limit_minor bigint not null);
create table if not exists s2_ordering.orders (
...
create table if not exists s2_ordering.order_lines (
    id bigserial primary key, order_id text not null references s2_ordering.orders(id),
    sku text not null, qty int not null, unit_price_minor bigint not null);
create table if not exists s2_ordering.outbox (
...
-- Inventory context
create table if not exists s2_inventory.stock_items (
    sku text primary key, available int not null);
create table if not exists s2_inventory.reservations (
    order_id text primary key, sku text not null, qty int not null, outcome text not null);

insert into s2_ordering.customers(id, name, credit_limit_minor)
values ('C1', 'Acme Buyer', 100000) on conflict (id) do nothing;
insert into s2_inventory.stock_items(sku, available)

...
import com.acme.samples.s1.ordering.domain.Order;
import com.acme.samples.s1.ordering.domain.OrderLine;
import com.acme.samples.s1.ordering.domain.OrderStatus;
import com.acme.samples.s1.ordering.domain.Orders;
...
/** Adapter implementing the Orders port over MyBatis-Plus. */
@Repository
public class OrderRepository implements Orders {
    ...
    public Optional<Order> byId(String id) {
        ...
        List<OrderLine> lines = lineMapper
                .selectList(new QueryWrapper<OrderLinePo>().eq("order_id", id))
                .stream()
                .map(l -> new OrderLine(l.getSku(), l.getQty(), l.getUnitPriceMinor()))
                .toList();
        return Optional.of(Order.rehydrate(
                po.getId(), po.getCustomerId(), lines,
                new Money(po.getTotalMinor(), po.getCurrency()),

package com.acme.samples.s1.ordering.infrastructure.persistence.order;
...
public class OrderRepository implements Orders {
    ...
    public Optional<Order> byId(String id) {
        ...
        List<OrderLineData> lines = lineMapper
                .selectList(new QueryWrapper<OrderLinePo>().eq("order_id", id))
                .stream()
                .map(l -> new OrderLineData(l.getSku(), l.getQty(), l.getUnitPriceMinor()))
                .toList();

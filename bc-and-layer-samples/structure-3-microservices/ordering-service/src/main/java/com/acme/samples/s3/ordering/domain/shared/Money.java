package com.acme.samples.s3.ordering.domain.shared;
...
public record Money(long amountMinor, String currency) {
    ...
    public static Money usd(long amountMinor) { return new Money(amountMinor, "USD"); }
    public Money plus(Money other) {
        if (!currency.equals(other.currency)) throw new IllegalArgumentException("currency mismatch");
        return new Money(amountMinor + other.amountMinor, currency);
    }
    public boolean lessThanOrEqual(Money other) { return this.amountMinor <= other.amountMinor; }
}

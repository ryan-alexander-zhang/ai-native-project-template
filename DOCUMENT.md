...
- After creating or substantively updating a `draft` doc, ask if it is reviewed. If yes, promote it: living docs to `active`, work items to `open`. Promoting a `spec`, `rule`, `design`, or `decision` writes `verified_against` ([docs/README.md](docs/README.md)). (In an autopilot run both rounds are replaced per `AUTOPILOT.md`.)

...
- When the work is done, set the work item to `resolved`; living docs stay `active`. A `plan` whose `implements` puts `spec`/`rule` items in scope needs its `docs/record/` acceptance checklist first.
